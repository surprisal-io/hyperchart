...
1. Project scope: nearest ancestor containing `.pi`, then `.pi/hypercharts/` below it.
2. User scope: `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/hypercharts/`.
3. Include flat `*.chart.ts`/`*.ts` files and bundle `<name>/chart.ts` entrypoints. Exclude `runs/`, hidden directories, and `node_modules/`.
4. Report chart name, scope, and absolute path. When both scopes define the same chart name, project scope wins.
5. Do not load every chart module merely to build the list. Chart modules execute top-level TypeScript when inspected. Inspect only selected chart with `hyperchart({ action: "inspect", chartPath })`.
...
4. Call `hyperchart` with `action: "inspect"` after every structural change.
...
1. Inspect the chart first with `hyperchart` with `action: "inspect"`.
