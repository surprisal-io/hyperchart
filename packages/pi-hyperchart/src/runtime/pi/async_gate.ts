...
export function createAsyncGate(limit: number): AsyncGate {
	if (!Number.isInteger(limit) || limit < 1) throw new Error("Async gate limit must be a positive integer");
	let active = 0;
	const waiting: Array<() => void> = [];
	return async <T>(operation: () => Promise<T>): Promise<T> => {
		if (active < limit) active += 1;
		else await new Promise<void>((resolveWaiting) => waiting.push(resolveWaiting));
		try {
