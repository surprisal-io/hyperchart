...
export class MemoryLogStore implements LogStore {
	...
	async captureSnapshot(branchId: BranchId): Promise<HistorySnapshot> {
		return { branchId, headSeqId: this.index.branch(branchId).headSeqId };
	}
	async listBranches(cursor?: BranchListCursor): Promise<BranchListChunk> {
		...
		return {
			items: page.map((entry) => this.index.branch(entry.branchId)),
			totalCount: creates.length,
			...(after.length > page.length
	...
	async cursorAt(input: {
		...
		return cursorAtItems(
			input.snapshot,
			input.subject,
			historyItemsForSubject(this.ancestryForSnapshot(input.snapshot), input.subject),
			input.seqId,
		);
	}
	...
	async findNearestCheckpoint(input: CheckpointQuery): Promise<OpaqueCheckpointEnvelope | undefined> {
		...
		const checkpoint = this.checkpoints
			...
			.sort(
				(left, right) =>
					(left.headSeqId === null ? Number.MAX_SAFE_INTEGER : distance.get(left.headSeqId)!) -
						(right.headSeqId === null ? Number.MAX_SAFE_INTEGER : distance.get(right.headSeqId)!) ||
	...
	> {
		return historyChunkFromItems(
			snapshot,
			subject,
			historyItemsForSubject(this.ancestryForSnapshot(snapshot), subject),
			cursor,
		);
	}
	...
	private enqueueCommit(
		expectedHeadSeqId: number | null | undefined,
		drafts: readonly DurableRecordDraft[],
		prepare?: PrepareStampedCommit,
		compareHead = false,
	): Promise<readonly DurableLogRecord[]> {
