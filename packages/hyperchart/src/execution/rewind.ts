...
export type RewindSelector = Readonly<{
	branchId: BranchId;
	state?: string;
	seqId?: number;
	to?: "compatible";
...
function recordMatchesState(records: readonly DurableLogRecord[], recordIndex: number, state: string): boolean {
	...
	return (
		record !== undefined &&
		semanticStatesForRecord(record, records, recordIndex).some(
			(path) => path === state || templatePath(path) === state || isUnderState(path, state),
		)
	);
}
function isUnderState(path: string, state: string): boolean {
	return (
		...
		path.startsWith(`${state}#`) ||
		templatePath(path).startsWith(`${state}.`)
	);
}
