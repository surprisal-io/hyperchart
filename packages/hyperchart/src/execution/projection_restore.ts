...
export function projectionContractForAst(ast: ChartAst): ProjectionContract {
	...
	return {
		...
		astDigest,
		selectorKey: `hyperchart-projection:${PROJECTOR_VERSION}:${astDigest}`,
	};
}
...
export async function loadBranchProjection(input: {
	...
	if (
		input.store.canStoreCheckpoints &&
		replayIsCheckpointable &&
		saveRequested &&
		decoded?.headSeqId !== snapshot.headSeqId
	) {
...
function decodeBranchProjection(value: unknown): BranchProjection | undefined {
	...
	if (
		...
		!isRecordOfJson(value.results) ||
		!isRecordOfPositiveIntegers(value.stateVisits) ||
		!isRecordOfNonEmptyStrings(value.sessions) ||
		!isArtifactPins(value.artifactPins) ||
		!(value.failure === undefined || isFailure(value.failure)) ||
		!isActors(value.actors) ||
		!isActorPools(value.actorPools) ||
...
function isPendingActions(value: unknown): boolean {
	return Array.isArray(value) && value.every(isPendingAction);
}
function isPendingAction(value: unknown): boolean {
	if (
		!isRecord(value) ||
		!isActionUid(value.actionUid) ||
		!isPositiveInteger(value.visitId) ||
		!isPositiveInteger(value.seqId) ||
		!isPositiveInteger(value.invokeSeqId) ||
		!isNonEmptyString(value.sessionId) ||
		!isRecord(value.definition) ||
		!isRecoveryCounters(value.recovery) ||
...
function isRecoveryCounters(value: unknown): boolean {
	...
	return [value.general, value.validation].every(
		(counter) =>
			...
			isNonNegativeInteger(counter.nudges) &&
...
function isLastRetry(value: unknown): boolean {
	return (
		...
		isNonNegativeInteger(value.nudgeAttempt) &&
		isNonNegativeInteger(value.restartAttempt) &&
...
function isArtifactPins(value: unknown): boolean {
	return (
		...
		Object.values(value).every(
			(pin) =>
				isExactRecord(pin, ["hash", "size"], []) &&
				/^[a-f0-9]{64}$/.test(String(pin.hash)) &&
				isNonNegativeInteger(pin.size),
		)
	);
}
function isFailure(value: unknown): boolean {
	return (
		isExactRecord(value, ["origin", "error", "seqId"], []) &&
		isNonEmptyString(value.origin) &&
		isPositiveInteger(value.seqId) &&
		isJsonValue(value.error)
	);
}
function isActors(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isActor);
}
function isActor(value: unknown): boolean {
	return (
		isExactRecord(
			...
			["owner", "currentMessageId"],
		) &&
		isPath(value.declaration) &&
		isPath(value.logicalOccurrence) &&
		isPath(value.occurrence) &&
		isPositiveInteger(value.generation) &&
		(value.owner === undefined || isPath(value.owner)) &&
		isJsonValue(value.input) &&
		isActorDefinition(value.definition, "actor") &&
		isPath(value.currentState) &&
		isStringArray(value.mailbox) &&
...
function isActorPools(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isActorPool);
}
function isActorPool(value: unknown): boolean {
	return (
		...
		(value.owner === undefined || isPath(value.owner)) &&
		isJsonValue(value.input) &&
		isActorDefinition(value.definition, "actorPool") &&
		isStringArray(value.mailbox) &&
...
function isLiveActorMessages(value: unknown): boolean {
	return (
		...
		Object.entries(value).every(
			([messageId, message]) =>
				messageId === (message as Record<string, unknown>)?.messageId && isProjectedMessage(message),
		)
	);
}
function isProjectedMessage(value: unknown): boolean {
	return (
		isExactRecord(
			value,
			["messageId", "event", "input", "producerState", "producerVisit", "batchIndex", "status"],
			["callId", "receiveState", "replyEvent", "replyOutput", "workerIndex"],
		) &&
		isNonEmptyString(value.messageId) &&
		isNonEmptyString(value.event) &&
		isJsonValue(value.input) &&
		isPath(value.producerState) &&
		isPositiveInteger(value.producerVisit) &&
		(value.callId === undefined || isNonEmptyString(value.callId)) &&
		isNonNegativeInteger(value.batchIndex) &&
		isOneOf(value.status, ["queued", "accepted", "replied", "settled", "failed", "cancelled"]) &&
		(value.receiveState === undefined || isPath(value.receiveState)) &&
		(value.replyEvent === undefined || isNonEmptyString(value.replyEvent)) &&
		(value.replyOutput === undefined || isJsonValue(value.replyOutput)) &&
		(value.workerIndex === undefined || isNonNegativeInteger(value.workerIndex))
	);
}
function isPendingActorCalls(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isPendingActorCall);
}
...
function isActorDefinition(value: unknown, kind: "actor" | "actorPool"): boolean {
	...
	if (kind === "actor") {
		return (
			isExactRecord(
				value,
				["kind", "name", "path", "input", "inputValue", "protocol", "initial", "states"],
				["owner"],
			) &&
	...
	return (
		isExactRecord(
			value,
			["kind", "name", "path", "input", "inputValue", "protocol", "concurrency", "worker"],
			["owner"],
		) &&
...
function isWorkerDefinition(value: unknown): boolean {
	return (
		isExactRecord(value, ["input", "protocol", "initial", "states"], []) &&
		isSchemaAst(value.input) &&
		isProtocol(value.protocol) &&
		isNonEmptyString(value.initial) &&
		isStateRecord(value.states)
...
function isProtocol(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every((entry) => isRecord(entry) && isJsonValue(entry));
}
function isStateRecord(value: unknown): boolean {
	return (
		...
		Object.values(value).every(
			(state) =>
				isRecord(state) &&
				isOneOf(state.kind, ["state", "receive", "send", "sendBatch", "call", "callBatch", "reply"]) &&
				isJsonValue(state),
		)
	);
}
function isSchemaAst(value: unknown): boolean {
	return (
		isExactRecord(value, ["kind", "schema"], ["runtimeContract"]) &&
		value.kind === "jsonSchema" &&
		isJsonValue(value.schema) &&
		(value.runtimeContract === undefined || isJsonValue(value.runtimeContract))
	);
}
function isChartEvent(value: unknown): boolean {
	return (
		isExactRecord(value, ["type"], ["output", "error"]) &&
		isNonEmptyString(value.type) &&
		(value.output === undefined || isJsonValue(value.output)) &&
		(value.error === undefined || isJsonValue(value.error))
	);
}
function isActionUid(value: unknown): boolean {
	return (
		isExactRecord(value, ["chart", "state", "action"], []) &&
		isNonEmptyString(value.chart) &&
		isPath(value.state) &&
		isNonEmptyString(value.action)
	);
}
function isCoordinates(value: Record<string, unknown>): boolean {
	return (
		isPositiveInteger(value.seqId) &&
		(value.parentId === null || isPositiveInteger(value.parentId)) &&
...
function projectionMatchesAst(projection: BranchProjection, ast: ChartAst): boolean {
	...
	if (
		!projection.pendingActions.every((pending) => {
			...
			return (
				state !== undefined &&
				matchesDeclaredUid(pending.actionUid, state.action.uid) &&
				pending.invokeSeqId <= pending.seqId &&
				pending.seqId <= projection.seqId &&
	...
	if (
		!Object.values(projection.pendingActorCalls).every((call) => {
			...
			return (
				...
				projectedEndpoint(projection, call.occurrence) !== undefined &&
...
function semanticStateExists(ast: ChartAst, projection: BranchProjection, path: string): boolean {
	return (
		(nodeAt(ast, path) !== undefined || actorContextForState(ast, path) !== undefined) &&
		concreteMapPathValid(ast, projection, path)
	);
}
...
function endpointMatchesAst(ast: ChartAst, projection: BranchProjection, endpoint: ProjectedActorOccurrence): boolean {
	...
	return (
		...
		context.occurrence === endpoint.occurrence &&
		...
		(endpoint.currentMessageId === undefined ||
			(!endpoint.mailbox.includes(endpoint.currentMessageId) &&
				current !== undefined &&
...
function poolMatchesAst(ast: ChartAst, projection: BranchProjection, endpoint: ProjectedActorPoolOccurrence): boolean {
	...
	for (const worker of endpoint.workers) {
		...
		if (worker.currentMessageId !== undefined) {
			...
			if (
				workerMessages.has(worker.currentMessageId) ||
				endpoint.mailbox.includes(worker.currentMessageId) ||
				current === undefined ||
				current.status === "queued" ||
				current.status === "settled" ||
				!messageCoordinatesMatchAst(ast, projection, current)
...
function endpointIdentityMatches(
	ast: ChartAst,
	declaration: ChartAst["actors"][string],
	...
	return (
		endpoint.logicalOccurrence === logical &&
		actorLogicalOccurrencePath(endpoint.occurrence, endpoint.generation) === logical &&
...
function concreteMapPathValidForOwner(projection: BranchProjection, owner: string, declarationOwner: string): boolean {
	...
	return key.length > 0 && Object.hasOwn(projection.spawns[container] ?? {}, key);
}
...
function messageCoordinatesMatchAst(
	ast: ChartAst,
	projection: BranchProjection,
...
function isRecordOfJsonRecords(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isJsonRecord);
}
function isRecordOfJson(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every((entry) => isJsonValue(entry));
}
function isRecordOfNonEmptyStrings(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isNonEmptyString);
}
function isRecordOfPositiveIntegers(value: unknown): boolean {
	return isRecord(value) && Object.values(value).every(isPositiveInteger);
}
...
function isHead(value: unknown): value is number | null {
...
function sameContract(left: ProjectionContract, right: ProjectionContract): boolean {
	return (
		...
		left.astDigest === right.astDigest &&
