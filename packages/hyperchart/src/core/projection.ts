...
export type ProjectedCancellation = {
	requestId: string;
...
export type BranchProjection = {
	...
	/** Cancellation quiescence is projected entirely from request/ack facts. */
	cancellations: Record<string, ProjectedCancellation>;
		...
		switch (record.type) {
			...
			case "cancellation":
				if (record.kind === "requested") {
					if (projection.cancellations[record.requestId] !== undefined) throw new Error(`Duplicate cancellation request ${record.requestId}`);
					projection.cancellations[record.requestId] = { requestId: record.requestId, target: record.target, acknowledged: false };
				} else {
					const cancellation = projection.cancellations[record.requestId];
					if (cancellation === undefined) throw new Error(`Cancellation acknowledgement ${record.requestId} has no request`);
					if (stableCancellationTarget(cancellation.target) !== stableCancellationTarget(record.target)) throw new Error(`Cancellation acknowledgement ${record.requestId} targets the wrong phase`);
					cancellation.acknowledged = true;
					applyCancellationAcknowledgement(projection, record.target);
				}
				break;
			case "actor_created": {
				...
				switch (record.kind) {
					...
					case "validated": {
						...
						if (retries !== undefined && validationAttempts > retries) {
							...
							// action pending so global quiescence requests and acknowledges its cancellation.
							break;
						}
...
function stableCancellationTarget(target: CancellationTarget): string {
	return JSON.stringify(target);
}

function applyCancellationAcknowledgement(projection: BranchProjection, target: CancellationTarget): void {
	if (target.kind === "action") {
		removePendingAction(projection, target.actionUid);
		return;
	}
	if (target.kind === "actor_call") {
		delete projection.pendingActorCalls[target.callId];
		return;
	}
	if (target.kind !== "actor_message") return;
	const actor = projection.actors[target.occurrence];
	if (actor === undefined) return;
	const message = actor.messages.find((entry) => entry.messageId === target.messageId);
	if (message !== undefined && message.status !== "settled") message.status = "cancelled";
	actor.mailbox = actor.mailbox.filter((entry) => entry.messageId !== target.messageId);
	if (actor.currentMessage?.messageId === target.messageId) delete actor.currentMessage;
