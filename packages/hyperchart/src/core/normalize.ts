...
function toChartArguments(
	...
	for (const [name, raw] of Object.entries(input)) {
		...
		for (const field of Object.keys(raw)) {
			if (field !== "description" && field !== "schema" && field !== "default") {
				diagnostics.push(
					diagnostic(
		...
		if (hasDefault) {
			defaultValue = toJsonValue(raw.default, `${pointer}/default`, diagnostics, source, new WeakSet());
		}
		if (hasDefault && defaultValue !== undefined && raw.schema instanceof z.ZodType) {
			try {
				const parsedDefault = raw.schema.safeParse(defaultValue);
				if (!parsedDefault.success) {
					diagnostics.push(
						diagnostic(
							"INVALID_CHART_ARGUMENT",
							`Chart argument '${name}' default does not match its schema: ${parsedDefault.error.issues.map((issue) => issue.message).join("; ")}`,
							`${pointer}/default`,
							source,
						),
					);
				...
				diagnostics.push(
					diagnostic(
						"INVALID_CHART_ARGUMENT",
						`Chart argument '${name}' default could not be checked against its schema: ${String(error)}`,
						`${pointer}/default`,
						source,
					),
				);
...
function toActorDeclarationAst(
	...
	if (input === undefined || inputValue === undefined || protocol === undefined) {
		return undefined;
	}
	if (isPool) {
		...
		const worker: ActorDefinitionAst = { input, protocol, initial, states: resolved };
		return deepFreeze({
			kind: "actorPool",
			name: placement.name,
			path: placement.path,
			...
			concurrency,
			worker,
		} satisfies ActorPoolDeclarationAst);
	}
	return deepFreeze({
		kind: "actor",
		name: placement.name,
...
function toProtocolAst(
	...
	for (const [event, raw] of Object.entries(input)) {
		...
		if (messageInput !== undefined) {
			protocol[event] = { input: messageInput, reply };
		}
	}
	return protocol;
}
...
function actorWorkflowSuccessors(node: ActorWorkflowStateAst): StateId[] {
	...
	if (node.kind === "send" || node.kind === "sendBatch" || node.kind === "callBatch") {
		return [node.target];
	}
	if (node.kind === "call") {
	...
	for (const [receiveId, receive] of Object.entries(states)) {
		...
		for (const [message, target] of Object.entries(receive.on)) {
			...
			const walk = (stateId: string): boolean => {
				...
				if (state === undefined || state.kind === "receive") {
					return false;
				}
				if (state.kind === "reply") {
...
function valueRefs(value: ValueAst | undefined): InputRef[] {
	...
	if (Array.isArray(value)) {
		return value.flatMap(valueRefs);
	}
	if (isInputRef(value)) {
		return [value];
	}
...
function schemaHasPath(schema: SchemaAst, path: string | undefined): boolean {
	...
	for (const segment of path.split(".")) {
		...
		} else if (isRecord(current.additionalProperties)) {
			current = current.additionalProperties;
		} else if (current.type === "array" && isRecord(current.items)) {
	...
	for (const [stateId, node] of Object.entries(states)) {
		...
		if (node.kind === "state") {
			...
			for (const [index, emitted] of (node.emit ?? []).entries()) {
				refs.push(...valueRefs(emitted.payload).map((ref) => ({ ref, pointer: `${pointer}/emit/${index}/payload` })));
			}
		...
		if (node.kind === "send") {
			refs.push(...valueRefs(node.input).map((ref) => ({ ref, pointer })));
		}
		...
		if (node.kind === "reply") {
			refs.push(...valueRefs(node.output).map((ref) => ({ ref, pointer })));
		}
...
function actorWorkflowDominators(
	...
	while (pending.length > 0) {
		...
		if (reachable.has(state) || states[state] === undefined) {
			continue;
		}
		reachable.add(state);
	...
	for (const state of reachable) {
		for (const target of actorWorkflowControlSuccessors(states[state]!)) {
			if (reachable.has(target)) {
				predecessors.get(target)!.add(state);
			}
		}
	}
	const dominators = new Map<StatePath, Set<StatePath>>();
	for (const state of reachable) {
		dominators.set(state, state === initial ? new Set([state]) : new Set(reachable));
	}
	let changed = true;
	while (changed) {
		changed = false;
...
function validateMessagingNode(
	placement: RawActorPlacement,
	node: SendStateAst | SendBatchStateAst | CallStateAst | CallBatchStateAst,
...
function validateCallRouting(
	...
	if (message === undefined || (node.kind !== "call" && node.kind !== "callBatch")) {
		return;
	}
...
function validateActorUsage(
	...
	for (const actor of Object.values(actors)) {
		for (const node of Object.values(actorDefinitionAst(actor).states)) {
			collect(node);
		}
	}
...
function collectState(
	...
	if (input.kind === "send" || input.kind === "sendBatch" || input.kind === "call" || input.kind === "callBatch") {
		if (role === "region") {
			diagnostics.push(diagnostic("INVALID_REGION", `Region '${path}' must be a compound state.`, pointer, source));
			return;
		}
...
export function declaredArtifactsForState(
	...
	if ("artifacts" in state.action) {
		add(state.action.artifacts as Readonly<Record<string, ArtifactAst>> | undefined);
	}
		...
		if (node.kind === "state") {
			...
			for (const [index, emitted] of (node.emit ?? []).entries()) {
				validateInputRefs(
					states,
					path,
					valueRefs(emitted.payload),
					`${pointer}/emit/${index}/payload`,
					diagnostics,
					source,
				);
			}
...
function validateEnterCycles(
	...
	const successors = (path: StatePath): StatePath[] => {
		...
		if (node.kind === "compound" || node.kind === "region") {
			return [`${path}.${node.initial}`];
		}
		if (node.kind === "parallel") {
			return node.regions.map((region) => `${path}.${region}`);
		}
	...
	const visit = (path: StatePath, trail: StatePath[]): void => {
		...
		entering.add(path);
		for (const next of successors(path)) {
			visit(next, [...trail, path]);
		}
	...
	for (const actor of Object.values(actors)) {
		...
		for (const ref of valueRefs(actor.inputValue)) {
			...
			if (ref.kind === "key" || ref.kind === "item") {
				let scope = consumer;
				while (scope !== undefined && states[scope]?.kind !== "map") {
					scope = parentStatePath(scope);
				}
...
function validateInputs(
	...
	for (const [path, node] of Object.entries(states)) {
		...
		if (
			...
			if (target in states) {
				edges.push({ target, pointer: `${pointer}/target` });
			}
		}
		if (node.kind === "state" && node.after !== undefined) {
			const target = sibling(node.after.target);
			if (target in states) {
				edges.push({ target, pointer: `${pointer}/after` });
			}
		}
		...
		if ((node.kind === "compound" || node.kind === "parallel" || node.kind === "map") && node.onDone.length > 0) {
			...
			if (target in states) {
				edges.push({ target, pointer: `${pointer}/onDone` });
			}
		}
	}
	...
	for (const [path, node] of Object.entries(states)) {
		if ((node.kind !== "state" && node.kind !== "map") || node.input === undefined) {
			continue;
		}
	...
	for (const [path, node] of Object.entries(states)) {
		...
		if (node.kind === "final") {
			for (const ref of node.notify?.prompt?.refs ?? []) {
				if (ref.kind === "result") {
					check(ref.state, path, `${pointer}/notify/prompt`, "result()");
				}
			}
		...
		if (node.kind === "state") {
			for (const template of actionTemplates(node.action)) {
				for (const ref of template.refs) {
					if (ref.kind === "result") {
						check(ref.state, path, `${pointer}/action`, "result()");
					}
				}
			}
			if (node.action.kind === "gate") {
			...
			for (const [index, emitted] of (node.emit ?? []).entries()) {
				for (const ref of valueRefs(emitted.payload)) {
					if (ref.kind === "result") {
						check(ref.state, path, `${pointer}/emit/${index}/payload`, "result()", true);
					}
				}
			}
			if (node.action.kind === "agent" && typeof node.action.reentry === "object") {
				for (const ref of node.action.reentry.resume.refs) {
					if (ref.kind === "result") {
						check(ref.state, path, `${pointer}/action/reentry/resume`, "result()");
					}
				}
			}
		...
		if (node.kind === "map") {
			if (node.over.kind === "result") {
				check(node.over.state, path, `${pointer}/over`, "result()");
			}
			if (typeof node.onReenter === "object") {
...
function buildDominanceGraph(
	...
	if (initial in states) {
		addEdge(DOMINATOR_START, initial);
	}
...
function reachableNodes(graph: Map<DominatorNode, Set<DominatorNode>>): Set<DominatorNode> {
	...
	for (let index = 0; index < queue.length; index++) {
		...
		if (node === undefined || reachable.has(node)) {
			continue;
		}
		reachable.add(node);
...
function dominatesForDataRef(
	...
	if (strictlyDominates(dominators, producer, consumer)) {
		return true;
	}
	return (
...
function parallelJoinDominates(
	...
	if (scope === undefined || underStateScope(consumer, scope.parallelPath)) {
		return false;
	}
	...
	if (!isDominatedBy(dominators, scope.parallelPath, consumer)) {
		return false;
	}
	if (consumer !== joinTarget && !strictlyDominates(dominators, joinTarget, consumer)) {
		return false;
	}
...
function mapJoinDominates(
	...
	while (mapPath !== undefined && states[mapPath]?.kind !== "map") {
		mapPath = parentStatePath(mapPath);
	}
	if (mapPath === undefined || underStateScope(consumer, mapPath)) {
		return false;
	}
	...
	if (!isDominatedBy(dominators, mapPath, consumer)) {
		return false;
	}
	if (consumer !== joinTarget && !strictlyDominates(dominators, joinTarget, consumer)) {
		return false;
...
function insideMap(
	...
	while (cur !== undefined) {
		if (states[cur]?.kind === "map" && (map === undefined || cur === map)) {
			return true;
		}
...
function enclosingMapScopes(states: Record<StatePath, StateAst>, producer: StatePath): StatePath[] {
	...
	while (current !== undefined) {
		if (states[current]?.kind === "map") {
			enclosingMaps.push(current);
		}
...
function retainsJoinOuterScope(states: Record<StatePath, StateAst>, producer: StatePath, consumer: StatePath): boolean {
	return enclosingMapScopes(states, producer)
		.slice(1)
		.every((map) => insideMap(states, consumer, map));
}
...
function isSafeJoinResultScope(states: Record<StatePath, StateAst>, producer: StatePath, consumer: StatePath): boolean {
	...
	return (
		joinedMap !== undefined &&
		!insideMap(states, consumer, joinedMap) &&
		outerMaps.every((map) => insideMap(states, consumer, map))
	);
}
...
function toEmits(
	...
	source: ChartSource,
	schemaRegistry: SchemaRegistry,
): readonly EmitAst[] | undefined {
	if (input === undefined) {
		return undefined;
	}
	if (!Array.isArray(input)) {
		diagnostics.push(diagnostic("INVALID_EMIT", "emit must be an array of emit() declarations.", path, source));
		return undefined;
	}
	const emits: EmitAst[] = [];
	for (const [index, raw] of input.entries()) {
		...
		if (!isRecord(raw)) {
			diagnostics.push(diagnostic("INVALID_EMIT", "emit entries must be objects.", pointer, source));
			continue;
		}
		if (typeof raw.event !== "string" || raw.event.length === 0) {
			diagnostics.push(
				diagnostic("INVALID_EMIT_EVENT", "Emit event must be a non-empty string.", `${pointer}/event`, source),
			);
		} else if (isReservedSystemEvent(raw.event)) {
			diagnostics.push(
				diagnostic(
					"RESERVED_EVENT_EMIT",
					`Emit event '${raw.event}' is reserved for system events.`,
					`${pointer}/event`,
					source,
				),
			);
		...
		for (const key of Object.keys(raw)) {
			if (key !== "event" && key !== "payload" && key !== "schema") {
				diagnostics.push(
					diagnostic("INVALID_EMIT", `Unknown emit field '${key}'.`, `${pointer}/${escapePointer(key)}`, source),
				);
			}
		}
		if (!Object.hasOwn(raw, "payload")) {
			diagnostics.push(diagnostic("INVALID_EMIT_PAYLOAD", "Emit payload is required.", `${pointer}/payload`, source));
		}
		const payload = toValueAst(raw.payload, `${pointer}/payload`, diagnostics, source);
		const schema = toSchemaAst(raw.schema, `${pointer}/schema`, diagnostics, source, schemaRegistry);
		emits.push({
	...
	switch (input.kind) {
		...
		case "gate": {
			if (typeof input.event !== "string" || input.event.length === 0) {
				diagnostics.push(
					diagnostic("INVALID_GATE_EVENT", "Gate event must be a non-empty string.", `${path}/event`, source),
				);
			} else if (isReservedSystemEvent(input.event)) {
				diagnostics.push(
					diagnostic(
						"RESERVED_EVENT_EMIT",
						`Gate event '${input.event}' is reserved for system events.`,
						`${path}/event`,
						source,
					),
				);
			...
			if (!Object.hasOwn(input, "payload")) {
				diagnostics.push(diagnostic("INVALID_GATE_PAYLOAD", "Gate payload is required.", `${path}/payload`, source));
			}
			if (Object.hasOwn(input, "prompt")) {
				diagnostics.push(diagnostic("INVALID_GATE_OPTION", "gate() does not accept prompt.", `${path}/prompt`, source));
			}
			if (Object.hasOwn(input, "options")) {
				diagnostics.push(
					diagnostic("INVALID_GATE_OPTION", "gate() does not accept options.", `${path}/options`, source),
				);
			}
			const payload = toValueAst(input.payload, `${path}/payload`, diagnostics, source);
			const reply = toSchemaAst(input.reply, `${path}/reply`, diagnostics, source, schemaRegistry);
			const uid: ActionUID = { chart: chartId, state: statePath, action: "gate" };
			return deepFreeze({
				kind: "gate",
				uid,
