...
export function hyperchartRunFromRuntime(
	...
	const actorOccurrences: HyperchartActorOccurrenceInfo[] = latestProjectedActors.map((actor) => {
		...
		const actorCancellations = Object.values(projection.cancellations).filter((cancellation) => {
			const target = cancellation.target;
			if (target.kind === "actor_effect" || target.kind === "actor_message") return target.occurrence === actor.occurrence;
			if (target.kind === "actor_call") return actor.messages.some((message) => message.callId === target.callId);
			return target.actionUid.state.startsWith(`${actor.occurrence}.`);
		});
		...
		return {
			...
			...(pending === undefined ? {} : { pendingCaller: { callId: pending.callId, state: pending.callerState, waitReason: pending.status === "accepted" ? "reply" as const : "accept" as const } }),
			...
			...(actorCancellations.length === 0 ? {} : { cancellation: { requested: true, acknowledged: actorCancellations.every((entry) => entry.acknowledged) } }),
		};
	});
...
function runtimeVisitHistories(
	...
	for (const record of records) {
		...
		if (record.type === "failure_intent") {
			const visit = histories.get(record.origin)?.at(-1);
			if (visit !== undefined && visit.status === "running") completeVisit(visit, { type: "FAILED", error: record.error }, record.timestamp);
			continue;
		}
		if (record.type !== "state_action" || skippedRecords.has(record)) continue;
		const stateId = record.actionUid.state;
...
function runtimeStateStatus(
	...
	// An acknowledged fail-fast actor-call cancellation invalidates its caller
	// even though the durable cancellation is projected separately from action
	// completion. Do not leave an impossible RUNNING call beside a failed actor.
	if (
		projection.failure !== undefined &&
		Object.values(projection.cancellations).some((cancellation) =>
			cancellation.acknowledged &&
			cancellation.target.kind === "actor_call" &&
			cancellation.target.callerState === runtimeStatePath)
	) return "failed";
