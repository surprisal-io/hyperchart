...
export function stateVisitHistoryItemToHost(
	item: StateVisitHistoryItem,
	session?: HyperchartAgentSessionInfo,
): HyperchartVisitInfo {
	const validations = item.records.filter(
		(record): record is Extract<DurableLogRecord, { type: "state_action"; kind: "validated" }> =>
			record.type === "state_action" && record.kind === "validated",
	);
	const completed = [...item.records]
		.reverse()
		.find(
			(record) =>
				...
				(record.type === "state_action" &&
					...
					(validations.length === 0 || record.event.type === "FAILED")),
		);
	const complete = item.records.find(
		(record): record is Extract<DurableLogRecord, { type: "state_action"; kind: "complete" }> =>
			record.type === "state_action" && record.kind === "complete",
	);
	const event =
	...
	return {
		...
		status:
			completed === undefined
				...
				: event === "FAILED"
					? "failed"
					: completed.type === "state_action" && completed.kind === "timer_fired"
						? "cancelled"
						: "done",
		...
		...(complete?.artifacts === undefined
			? {}
			: {
					artifactPins: Object.entries(complete.artifacts).map(([path, pin]) => ({
						path,
						hash: pin.hash,
						size: pin.size,
					})),
...
export function mapVisitHistoryItemToHost(item: MapVisitHistoryItem): HyperchartMapVisitInfo {
	return {
		visit: item.visit,
		spawnSeqId: item.seqId,
		startedAt: item.spawn.timestamp,
		instances: { ...item.spawn.instances },
	};
}

export function actorGenerationHistoryItemToHost(item: ActorGenerationHistoryItem): HyperchartActorGenerationInfo {
	return {
		logicalOccurrence: item.logicalOccurrence,
		occurrencePath: item.created.occurrence,
		generation: item.created.generation,
		createdSeqId: item.seqId,
		createdAt: item.created.timestamp,
	};
}

export function actorMessageHistoryItemToHost(
	item: ActorMessageHistoryItem,
	ast?: ChartAst,
	...
	const batch = {
		...
		messages: item.enqueued.messages.map((message): HyperchartActorMessageInfo => {
			...
			const replied = records.find(
				(record): record is Extract<DurableLogRecord, { type: "actor_message"; kind: "replied" }> =>
			...
			return {
				...
				...(replied === undefined
					...
					: {
							...
							...(replied.replyEvent === undefined ? {} : { replyEvent: replied.replyEvent }),
							...(Object.hasOwn(replied, "output") ? { replyOutput: replied.output } : {}),
						}),
			};
		}),
	};
...
function enrichActorMessageBatch(
	batch: HyperchartActorMessageBatchInfo,
	item: ActorMessageHistoryItem,
	ast: ChartAst,
	ancestry: readonly DurableLogRecord[],
): HyperchartActorMessageBatchInfo {
...
export function actorMessageHistoryItemsToHost(
	items: readonly ActorMessageHistoryItem[],
	ast: ChartAst,
	ancestry: readonly DurableLogRecord[],
	base = items.map((item) => actorMessageHistoryItemToHost(item)),
): readonly HyperchartActorMessageBatchInfo[] {
	...
	for (const record of ancestry) {
		if (record.type === "actor_message") {
			...
			if (message !== undefined && logicalOccurrence !== undefined && record.kind === "accepted") {
				...
				message.acceptedAt = record.timestamp;
				message.receiveState = record.receiveState.replace(`${record.occurrence}.`, `${logicalOccurrence}.`);
				if (record.workerIndex !== undefined) {
					message.workerIndex = record.workerIndex;
					message.workerOccurrencePath = actorPoolWorkerOccurrencePath(record.occurrence, record.workerIndex);
				}
	...
	return items.map((item, index) => ({
		...base[index]!,
		messages: base[index]!.messages.map(
			(message) => messages.get(`${item.occurrence}\0${message.messageId}`)?.info ?? message,
		),
	}));
}
...
function invocationInfo(action: StateActionAst): HyperchartVisitInfo["invocation"] {
	switch (action.kind) {
		case "agent":
			return { kind: "agent", ...(action.task === undefined ? {} : { task: templatePreview(action.task) }) };
		case "script":
			return { kind: "script", command: action.command, args: [...action.args] };
		case "tsImport":
