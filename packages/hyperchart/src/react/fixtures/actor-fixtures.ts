...
function created(
	...
	return {
		...
		input: resolvedInput ?? definition.inputValue,
		definition,
		...stamp(seqId),
	};
}
function envelope(
	...
	const input =
		definition.kind === "call" || definition.kind === "send"
			? definition.input
...
function enqueue(
	...
	return {
		...
		generation,
		source: {
			producerState: sourcePath,
			kind: definition.kind,
			definition,
			targetDeclaration,
			event,
			inputSchema: messageContract(ast, targetDeclaration, event).input,
		},
		messages,
		...stamp(seqId),
	};
...
function buildRun(
	name: string,
	ast: ChartAst,
	inspect: typeof actorInspectorInspectResult,
	...
	return hyperchartRunFromRuntime(inspect, ast, records, {
		...
		status: {
			...
			state: status,
			startedAt: now,
			updatedAt: now + records.length * 1_000,
...
export const actorBusyFifoRun = buildRun(
	"busy-fifo",
	actorInspectorAst,
	actorInspectorInspectResult,
...
export const actorSelfChart = chart({
	kind: "chart",
	id: "actor-self-send-story",
	actors: { workers: selfWorkers },
	initial: "start",
...
const callScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-call-story",
		actors: { editor: callEditor },
		initial: "apply",
		states: {
			apply: call({
				to: callEditor,
				event: "APPLY",
				input: { patch: "follow-up" },
...
const callBaseRecords: DurableLogRecord[] = [
	{ type: "args", args: {}, ...stamp(1) },
	created(callAst, "@editor", "@editor", 2),
];
...
export const actorPendingCallRun = callScenario.runtimeRun(
	capturedStorySchedule(callScenario.ast, pendingCallRecords),
	{
		runId: "actor:pending-call",
		status: { state: "running", updatedAt: now + 3_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 3_000,
	},
...
const actorNamedReplySchedule: DurableLogRecord[] = [
	...
	},
	{
		...
		actionUid: editorApplyUid,
		definition: editorApply.action,
		...stamp(5),
	},
	{ type: "state_action", kind: "complete", actionUid: editorApplyUid, event: { type: "DONE" }, ...stamp(6) },
	{
		...
		occurrence: "@editor",
		messageId: callMessage.messageId,
		message: editorSettle.message,
		replyEvent: editorSettle.event,
		output: editorSettle.output,
		schema: appliedSchema,
		...stamp(7),
	},
];
export const actorNamedReplyRecords = capturedStorySchedule(callScenario.ast, actorNamedReplySchedule);
export const actorNamedReplyRun = callScenario.runtimeRun(
	capturedStorySchedule(callScenario.ast, actorNamedReplySchedule),
	{
		runId: "actor:named-reply",
		status: { state: "running", updatedAt: now + 7_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 7_000,
	},
...
const drainScenario = storyScenario(
	chart({
		kind: "chart",
			...
			phase: {
				...
				states: {
					dispatch: sendBatch({
						to: drainWorker,
						event: "RECORD",
						inputs: [{ path: "one.log" }, { path: "two.log" }, { path: "three.log" }, { path: "four.log" }],
						target: "finished",
					}),
					finished: final(),
				},
...
const drainingRecords: DurableLogRecord[] = [
	...
	enqueue(drainAst, "phase.dispatch", "phase.@worker", "RECORD", drainMessages, 3),
	{
		...
		messageId: drainMessages[0]!.messageId,
...
export const actorFailureRun = callScenario.runtimeRun(capturedStorySchedule(callScenario.ast, failureRecords), {
	runId: "actor:failure",
	status: { state: "failed", updatedAt: now + 5_000 },
	cwd: "/workspace",
	createdAt: now,
	updatedAt: now + 5_000,
});
...
const changedActorScenario = storyScenario(
	chart({
		kind: "chart",
		...
		states: {
			queue: sendBatch({
				to: changedEditor,
				event: "APPLY",
				inputs: [{ patch: "first patch" }, { patch: "second patch" }],
				target: "apply-call",
			}),
			"apply-call": call({
				to: changedEditor,
...
const compatibilityWarnings = [
	...(compatibility.broken === undefined
		? []
		: [`Broken replay at seq ${compatibility.broken.seqId}: ${compatibility.broken.error}`]),
...
export const actorBrokenReplayRun = hyperchartRunFromRuntime(
	...
	baseEditorRecords,
	{
		runId: "actor:broken-replay",
		status: { state: "failed", updatedAt: now + 2_000, replayWarnings: compatibilityWarnings },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 2_000,
	},
...
const auditScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-send-void-story",
		actors: { auditor },
		initial: "record",
		states: {
...
const auditRecords: DurableLogRecord[] = [
	{ type: "args", args: {}, ...stamp(1) },
	created(auditAst, "@auditor", "@auditor", 2),
	enqueue(auditAst, "record", "@auditor", "RECORD", [auditMessage], 3),
	{
		type: "actor_message",
		kind: "accepted",
		occurrence: "@auditor",
		messageId: auditMessage.messageId,
		receiveState: "@auditor.idle",
		...stamp(4),
	},
	{
		type: "actor_message",
	...
	{ type: "actor_scope", kind: "closing", occurrence: "@auditor", ...stamp(7) },
	{ type: "actor_scope", kind: "stopped", occurrence: "@auditor", ...stamp(8) },
];
export const actorSendVoidRun = auditScenario.runtimeRun(capturedStorySchedule(auditScenario.ast, auditRecords), {
	runId: "actor:send-void",
	status: { state: "complete", updatedAt: now + 8_000 },
	cwd: "/workspace",
	createdAt: now,
	updatedAt: now + 8_000,
});
...
const reentryScenario = storyScenario(
	chart({
		kind: "chart",
...
const reentryMessage3 = {
	...envelope(reentryAst, "phase.record", "phase.record", "phase.record:message:3:0", 0),
	producerVisit: 3,
};
const reentryRecords: DurableLogRecord[] = [
	{
		type: "actor_created",
		declaration: "phase.@auditor",
		occurrence: "phase.@auditor",
		generation: 1,
		owner: "phase",
		input: reentryDefinition.inputValue,
		definition: reentryDefinition,
		...stamp(1),
	},
	enqueue(reentryAst, "phase.record", "phase.@auditor", "RECORD", [reentryMessage1], 2),
	{
		...
		kind: "accepted",
		occurrence: "phase.@auditor",
		messageId: reentryMessage1.messageId,
		receiveState: "phase.@auditor.idle",
		...stamp(3),
	},
	...
	{ type: "actor_scope", kind: "stopped", occurrence: "phase.@auditor", ...stamp(7) },
	{
		...
		kind: "invoke",
		sessionId: "session-id",
		actionUid: reentryAction.action.uid,
		definition: reentryAction.action,
		...stamp(8),
	},
	{
		type: "state_action",
	...
	enqueue(reentryAst, "phase.record", "phase.@auditor~2", "RECORD", [reentryMessage2], 11, "phase.record", 2),
	{
		...
		kind: "accepted",
		occurrence: "phase.@auditor~2",
		messageId: reentryMessage2.messageId,
		receiveState: "phase.@auditor~2.idle",
		...stamp(12),
	},
	...
	enqueue(reentryAst, "phase.record", "phase.@auditor~3", "RECORD", [reentryMessage3], 20, "phase.record", 3),
	{
		...
		kind: "accepted",
		occurrence: "phase.@auditor~3",
		messageId: reentryMessage3.messageId,
		receiveState: "phase.@auditor~3.idle",
		...stamp(21),
	},
...
export const actorReentryRun = reentryScenario.runtimeRun(capturedStorySchedule(reentryScenario.ast, reentryRecords), {
	runId: "actor:reentry",
	status: { state: "running", updatedAt: now + 26_000 },
	cwd: "/workspace",
	createdAt: now,
...
const mapScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-map-story",
		args: { projects: {} },
			...
			projects: map({
				...
				states: {
					apply: call({
						...
						event: "APPLY",
						input: { patch: "p" },
						transitions: { APPLIED: "finished", REJECTED: "finished" },
					}),
					finished: final(),
				},
			}),
...
const mapCallA = envelope(
	mapAst,
	"projects.apply",
	"projects#a.apply",
	"projects#a.apply:message:1:0",
	0,
	"projects#a.apply:call:1",
);
const mapCallB = envelope(
	mapAst,
	"projects.apply",
	"projects#b.apply",
	"projects#b.apply:message:1:0",
	0,
	"projects#b.apply:call:1",
);
...
const mapRecords: DurableLogRecord[] = [
	...
	enqueue(mapAst, "projects#b.apply", "projects#b.@editor", "APPLY", [mapCallB], 6, "projects.apply"),
	{
		type: "actor_message",
		kind: "accepted",
		occurrence: "projects#b.@editor",
		messageId: mapCallB.messageId,
		receiveState: "projects#b.@editor.idle",
		...stamp(7),
	},
];
export const actorMapPartialRun = mapScenario.runtimeRun(
	capturedStorySchedule(mapScenario.ast, mapRecords.slice(0, 3)),
	{
		runId: "actor:map-partial",
		status: { state: "running", updatedAt: now + 3_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 3_000,
...
export const actorMapLocalRun = mapScenario.runtimeRun(capturedStorySchedule(mapScenario.ast, mapRecords), {
	runId: "actor:map-local",
	status: { state: "running", updatedAt: now + 4_000 },
	cwd: "/workspace",
	createdAt: now,
	updatedAt: now + 4_000,
});
...
const PoolWorkProtocol = protocol({
	WORK: message({
		input: z.object({ id: z.number(), label: z.string() }).strict(),
		reply: z.object({ id: z.number(), receipt: z.string() }).strict(),
	}),
});
const PoolWorker = actor({
	...
	states: {
		...
		work: {
			kind: "state",
			action: agent("pool-worker", {
				task: "Process the assigned pool item.",
				reply: z.object({ id: z.number(), receipt: z.string() }).strict(),
			}),
			transitions: { DONE: "settle" },
		},
...
const poolWorkerUid = (index: number) => ({
	...poolAction.action.uid,
	state: poolAction.action.uid.state.replace(".$worker.", `.$worker-${index}.`),
});
const poolAccepted = (messageIndex: number, workerIndex: number, seqId: number): DurableLogRecord => ({
	type: "actor_message",
	kind: "accepted",
	occurrence: "@workers",
	messageId: poolMessages[messageIndex]!.messageId,
	receiveState: `@workers.$worker-${workerIndex}.idle`,
	workerIndex,
	...stamp(seqId),
});
const poolInvoked = (workerIndex: number, seqId: number): DurableLogRecord => ({
	...
	kind: "invoke",
	sessionId: "session-id",
	actionUid: poolWorkerUid(workerIndex),
	definition: poolAction.action,
	...stamp(seqId),
});
const poolCompleted = (workerIndex: number, messageIndex: number, seqId: number): DurableLogRecord => ({
	...
	kind: "complete",
	actionUid: poolWorkerUid(workerIndex),
	event: { type: "DONE", output: { id: messageIndex, receipt: `receipt-${messageIndex}` } },
	...stamp(seqId),
});
const poolReplied = (messageIndex: number, workerIndex: number, seqId: number): DurableLogRecord => ({
	...
	messageId: poolMessages[messageIndex]!.messageId,
	message: "WORK",
	output: { id: messageIndex, receipt: `receipt-${messageIndex}` },
	schema: poolReplyContract.schema,
	workerIndex,
	...stamp(seqId),
});
...
const poolPartialRecords: DurableLogRecord[] = [...actorPoolBusySchedule, poolReplied(1, 1, 9), poolSettled(1, 1, 10)];
const actorPoolCompleteSchedule: DurableLogRecord[] = [
	...
	poolSettled(3, 0, 23),
	{
		...
		callId: "batch:call:1",
		callerState: "batch",
		messageIds: poolMessages.map((message) => message.messageId),
		...stamp(24),
	},
...
const poolSessionProgress = {
	...
	sessions: {
		"actor-pool-story:@workers.$worker-0.work:work:1:6": {
			actionUid: poolWorkerUid(0),
			visit: 1,
			status: "completed",
			startedAt: now + 6_000,
			completedAt: now + 16_000,
			model: "storybook/pool-worker",
			turnCount: 2,
			lastMessage: "Finished alpha.",
		},
		"actor-pool-story:@workers.$worker-1.work:work:1:7": {
			actionUid: poolWorkerUid(1),
			visit: 1,
			status: "completed",
			startedAt: now + 7_000,
			...
			model: "storybook/pool-worker",
			turnCount: 1,
			lastMessage: "Finished beta first.",
		},
		"actor-pool-story:@workers.$worker-1.work:work:2:12": {
			actionUid: poolWorkerUid(1),
			visit: 2,
			status: "completed",
			...
			model: "storybook/pool-worker",
			turnCount: 1,
			lastMessage: "Reused worker for gamma.",
		},
	},
};
...
export const actorPoolIdleRun = poolScenario.runtimeRun(capturedStorySchedule(poolScenario.ast, poolBaseRecords), {
	runId: "actor:pool-idle",
	status: { state: "running", updatedAt: now + 2_000 },
	cwd: "/workspace",
	createdAt: now,
	updatedAt: now + 2_000,
});
export const actorPoolBusyRun = poolScenario.runtimeRun(
	capturedStorySchedule(poolScenario.ast, actorPoolBusySchedule),
	{
		runId: "actor:pool-busy",
		status: { state: "running", updatedAt: now + 8_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 8_000,
		sessionProgress: poolSessionProgress,
	},
);
export const actorPoolPartialBatchRun = poolScenario.runtimeRun(
	capturedStorySchedule(poolScenario.ast, poolPartialRecords),
	{
		runId: "actor:pool-partial",
		status: { state: "running", updatedAt: now + 10_000 },
		cwd: "/workspace",
		createdAt: now,
...
export const actorPoolOutOfOrderRun = poolScenario.runtimeRun(
	capturedStorySchedule(poolScenario.ast, actorPoolCompleteSchedule),
	{
		runId: "actor:pool-complete",
		status: { state: "complete", updatedAt: now + 26_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 26_000,
		sessionProgress: poolSessionProgress,
...
export const actorPoolCrowdedChart = chart({
	...
	states: {
		batch: callBatch({
			...
			event: "WORK",
			inputs: [
				{ id: 0, label: "queued-item-0" },
				{ id: 1, label: "queued-item-1" },
				{ id: 2, label: "queued-item-2" },
				{ id: 3, label: "queued-item-3" },
				{ id: 4, label: "queued-item-4" },
				{ id: 5, label: "queued-item-5" },
				{ id: 6, label: "queued-item-6" },
				{ id: 7, label: "queued-item-7" },
				{ id: 8, label: "queued-item-8" },
				{ id: 9, label: "queued-item-9" },
			],
			target: "done",
		}),
		done: final(),
...
const crowdedWorkerUid = (index: number) => ({
	...crowdedAction.action.uid,
	state: crowdedAction.action.uid.state.replace(".$worker.", `.$worker-${index}.`),
});
const crowdedAccepted = (messageIndex: number, workerIndex: number, seqId: number): DurableLogRecord => ({
	type: "actor_message",
	kind: "accepted",
	occurrence: "@workers",
	messageId: crowdedMessages[messageIndex]!.messageId,
	receiveState: `@workers.$worker-${workerIndex}.idle`,
	workerIndex,
	...stamp(seqId),
});
const crowdedInvoked = (workerIndex: number, seqId: number): DurableLogRecord => ({
	...
	kind: "invoke",
	sessionId: "session-id",
	actionUid: crowdedWorkerUid(workerIndex),
	definition: crowdedAction.action,
	...stamp(seqId),
});
...
const crowdedReplied = (messageIndex: number, workerIndex: number, seqId: number): DurableLogRecord => ({
	...
	message: "WORK",
	output: { id: messageIndex, receipt: `receipt-${messageIndex}` },
	schema: crowdedReplyContract.schema,
	workerIndex,
	...stamp(seqId),
});
...
export const actorPoolCrowdedRun = crowdedScenario.runtimeRun(
	capturedStorySchedule(crowdedScenario.ast, actorPoolCrowdedSchedule),
	{
...
const poolDrainScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-pool-drain-story",
		initial: "phase",
		states: {
			phase: {
				kind: "compound",
				actors: { workers: drainingPool },
				initial: "dispatch",
				onDone: "done",
				states: {
					dispatch: sendBatch({
...
const poolDrainRecords: DurableLogRecord[] = [
	...
	enqueue(poolDrainAst, "phase.dispatch", "phase.@workers", "WORK", poolDrainMessages, 3),
	{
		type: "actor_message",
		kind: "accepted",
		occurrence: "phase.@workers",
		messageId: poolDrainMessages[0]!.messageId,
		receiveState: "phase.@workers.$worker-0.idle",
		workerIndex: 0,
		...stamp(4),
	},
	{
		...
		kind: "accepted",
		occurrence: "phase.@workers",
		messageId: poolDrainMessages[1]!.messageId,
		receiveState: "phase.@workers.$worker-1.idle",
		workerIndex: 1,
		...stamp(5),
	},
	{ type: "actor_scope", kind: "closing", occurrence: "phase.@workers", ...stamp(6) },
];
export const actorPoolDrainingRun = poolDrainScenario.runtimeRun(
	capturedStorySchedule(poolDrainScenario.ast, poolDrainRecords),
	{
		runId: "actor:pool-draining",
		status: { state: "running", updatedAt: now + 6_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 6_000,
	},
...
const mapPoolScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-pool-map-reentry-story",
		args: { projects: {} },
		initial: "projects",
		states: {
			projects: map({
				over: arg("projects"),
				actors: { workers: mapPool },
				initial: "dispatch",
				onDone: "between",
				states: {
					dispatch: send({
						to: mapPool,
						event: "WORK",
						input: { id: 0, label: "map-generation-work" },
						target: "hold",
					}),
...
const mapPoolMessage = envelope(
	...
	"projects.dispatch",
	"projects#a.dispatch",
	"projects#a.dispatch:message:1:0",
	0,
);
const mapPoolRecords: DurableLogRecord[] = [
	...
	),
	{
		type: "actor_message",
		kind: "accepted",
		occurrence: "projects#a.@workers",
		messageId: mapPoolMessage.messageId,
		receiveState: "projects#a.@workers.$worker-0.idle",
		workerIndex: 0,
		...stamp(5),
	},
	{
		...
		kind: "invoke",
		sessionId: "session-id",
		actionUid: mapPoolWorkerUid,
		definition: mapPoolWorkerAction.action,
		...stamp(6),
	},
	{
		type: "state_action",
		kind: "complete",
		actionUid: mapPoolWorkerUid,
		event: { type: "DONE", output: { id: 0, receipt: "map-generation-receipt" } },
		...stamp(7),
	},
	{
		type: "actor_message",
		...
		messageId: mapPoolMessage.messageId,
		message: "WORK",
		output: { id: 0, receipt: "map-generation-receipt" },
		schema: mapPoolReply.schema,
		workerIndex: 0,
		...stamp(8),
	},
	{
	...
	},
	{
		...
		kind: "invoke",
		sessionId: "session-id",
		actionUid: mapPoolHoldUid,
		definition: mapPoolHold.action,
		...stamp(10),
	},
	...
	{ type: "actor_scope", kind: "stopped", occurrence: "projects#a.@workers", ...stamp(13) },
	{
		...
		kind: "invoke",
		sessionId: "session-id",
		actionUid: mapPoolBetween.action.uid,
		definition: mapPoolBetween.action,
		...stamp(14),
	},
	{
		type: "state_action",
...
export const actorPoolMapReentryRun = mapPoolScenario.runtimeRun(
	capturedStorySchedule(mapPoolScenario.ast, mapPoolRecords),
	{
		runId: "actor:pool-map-reentry",
		status: { state: "running", updatedAt: now + 17_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 17_000,
	},
);
...
const OverflowProtocol = protocol({
	PROCESS_WITH_A_LONG_PROTOCOL_EVENT_NAME: message({
		input: z
			...
			.strict(),
		replies: {
			OUTCOME_1: z.object({ result: z.string() }),
			OUTCOME_2: z.object({ result: z.string() }),
			OUTCOME_3: z.object({ result: z.string() }),
			OUTCOME_4: z.object({ result: z.string() }),
			OUTCOME_5: z.object({ result: z.string() }),
			OUTCOME_6: z.object({ result: z.string() }),
		},
	}),
});
...
const overflowInput = (index: number) => ({
	...
	field2: `value-${index}-2`,
	field3: `value-${index}-3`,
	field4: `value-${index}-4`,
	field5: `value-${index}-5`,
	field6: `value-${index}-6`,
	field7: `value-${index}-7`,
	field8: `value-${index}-8`,
});
const overflowActor = OverflowActor({ file: "overflow.ts" });
const overflowScenario = storyScenario(
	chart({
		kind: "chart",
		id: "actor-overflow-story",
		actors: { overflow: overflowActor },
		initial: "queue",
		states: {
			queue: sendBatch({
				to: overflowActor,
				event: "PROCESS_WITH_A_LONG_PROTOCOL_EVENT_NAME",
				inputs: Array.from({ length: 50 }, (_, index) => overflowInput(index)) as [
					ReturnType<typeof overflowInput>,
					...ReturnType<typeof overflowInput>[],
				],
				target: "done",
			}),
...
export const actorOverflowRun = overflowScenario.runtimeRun(
	capturedStorySchedule(overflowScenario.ast, overflowRecords),
	{
		runId: "actor:overflow",
		status: { state: "running", updatedAt: now + 3_000 },
		cwd: "/workspace",
		createdAt: now,
		updatedAt: now + 3_000,
