...
function created(ast: ChartAst, declarationPath: string, occurrence: string, seqId: number, owner?: string, resolvedInput?: unknown): DurableLogRecord {
	...
	return { type: "actor_created", declaration: declarationPath, logicalOccurrence: occurrence, occurrence, generation: 1, ...(owner === undefined ? {} : { owner }), input: resolvedInput ?? definition.inputValue, definition, ...stamp(seqId) };
}
...
const failureRequest = "storybook:actor-message-cancel";
const failedTarget = { kind: "actor_message" as const, occurrence: "@editor", messageId: callMessage.messageId };
const callTarget = { kind: "actor_call" as const, callId: "apply:1", callerState: "apply" };
const failureRecords: DurableLogRecord[] = [
	...
	{ type: "cancellation", kind: "requested", requestId: failureRequest, target: failedTarget, ...stamp(6) },
	{ type: "cancellation", kind: "acknowledged", requestId: failureRequest, target: failedTarget, ...stamp(7) },
	{ type: "cancellation", kind: "requested", requestId: `${failureRequest}:call`, target: callTarget, ...stamp(8) },
	{ type: "cancellation", kind: "acknowledged", requestId: `${failureRequest}:call`, target: callTarget, ...stamp(9) },
];
export const actorFailureRun = callScenario.runtimeRun(failureRecords, { runId: "actor:failure", status: { state: "failed", updatedAt: now + 9_000 }, cwd: "/workspace", createdAt: now, updatedAt: now + 9_000 });
...
const reentryRecords: DurableLogRecord[] = [
	{ type: "actor_created", declaration: "phase.@auditor", logicalOccurrence: "phase.@auditor", occurrence: "phase.@auditor", generation: 1, owner: "phase", input: reentryDefinition.inputValue, definition: reentryDefinition, ...stamp(1) },
	...
	{ type: "actor_created", declaration: "phase.@auditor", logicalOccurrence: "phase.@auditor", occurrence: "phase.@auditor~2", generation: 2, owner: "phase", input: reentryDefinition.inputValue, definition: reentryDefinition, ...stamp(10) },
	...
	{ type: "actor_created", declaration: "phase.@auditor", logicalOccurrence: "phase.@auditor", occurrence: "phase.@auditor~3", generation: 3, owner: "phase", input: reentryDefinition.inputValue, definition: reentryDefinition, ...stamp(19) },
