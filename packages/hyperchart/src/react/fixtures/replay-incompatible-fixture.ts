...
const definition = (validated: boolean) => chart({
	...
	states: {
		experiment: {
			kind: "state", action: validated ? agent("experimenter", { task: t`Investigate ${arg("topic")}` }) : script("true"),
			...(validated ? { validate: tsImport("./checks.js", "ok") } : {}),
		...
		suffix: parallel({ states: {
			guarded: { kind: "compound", initial: "work", states: {
				work: { kind: "state", action: agent("candidate-writer", { artifacts: { candidate: "candidate.txt" } }), validate: tsImport("./checks.js", "candidate"), transitions: { DONE: "done" } },
				done: final(),
			} },
