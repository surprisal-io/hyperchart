...
const definition = (validated: boolean) => chart({
	kind: "chart", id: "removed-validator", initial: "experiment",
	states: {
		experiment: {
			kind: "state", action: validated ? agent("experimenter", { task: t`Investigate ${arg("topic")}`,
							validation: { guard: tsImport("./checks.js", "ok") },
						})
					: script("true"),
				transitions: { DONE: { target: "after", input: { topic: arg("topic") } } },
		},
		after: { kind: "state", input: { topic: z.string() }, action: agent("recorder", { task: t`Record ${input("topic")}` }), transitions: { AGAIN: { target: "after", input: { topic: arg("topic") } }, DONE: "persistScript" } },
		...
		suffix: parallel({ states: {
			guarded: { kind: "compound", initial: "work", states: {
				work: { kind: "state", action: agent("candidate-writer", { artifacts: { candidate: "candidate.txt" },
									validation: { guard: tsImport("./checks.js", "candidate") },
								}),
								transitions: { DONE: "done" } },
				done: final(),
			} },
			clock: { kind: "compound", initial: "work", states: {
...
export function replayIncompatibleStoryRun() {
	...
	if (broken === undefined || records[0] === undefined || records.at(-1) === undefined) throw new Error("Changed action identity must break replay");
	return hyperchartRunFromReplayIncompatibility(changedReplayScenario.inspect, broken, { runId: "removed-validator", cwd: "/workspace", createdAt: records[0].timestamp, updatedAt: records.at(-1)!.timestamp });
}
