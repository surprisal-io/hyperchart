import { z } from "zod";
import { actor, agent, completion, final, message, messageInput, notify, protocol, receive, reply, send, waitFor } from "../../core/dsl.js";
...
const bound = refs<
	Record<never, never>, { wait: { value: string } }, Record<never, never>, Record<never, never>,
	Record<never, Record<string, unknown>>, { worker: typeof Work },
...
const Worker = actor({
	...
	states: {
		...
		publish: notify({ to: doneRef, event: "DONE", payload: { value: messageInput("START", "value") }, target: "settle" }),
		settle: reply({ target: "idle" }),
	},
});
const worker = Worker({});
export const completionStoryChart: ChartCst = bound.chart({
	kind: "chart", id: "storybook-chart-completion",
	completions: { done: completion({ event: "DONE", schema: z.object({ value: z.string() }).strict() }) },
	actors: { worker }, initial: "dispatch",
	states: {
			...
			switch (effect.kind) {
				case "durable_records": {
					const added = effect.records.map((draft): DurableLogRecord => ({
						...
						branchId: this.branchId, timestamp: this.startedAt + this.seqId * 1_000,
					}) as DurableLogRecord);
					this.records.push(...added);
					const reached = this.snapshot === "notification-retained"
						? added.some((record) => record.type === "completion" && record.kind === "notified")
						: this.snapshot === "waiting"
							? added.some((record) => record.type === "state_action" && record.kind === "invoke" && record.actionUid.state === "wait")
							: this.snapshot === "notify-in-flight"
								? added.some((record) => record.type === "state_action" && record.kind === "invoke" && record.actionUid.state === "@worker.publish")
				...
				case "completion_notify":
					this.push({ kind: "completion_effect", effectId: effect.id, operation: "notify",
				...
				case "agent":
					...
					break;
				case "cancel": break;
				default: throw new Error(`Unexpected completion story effect ${effect.kind}`);
			}
		}
	}
...
async function capture(snapshot: Snapshot): Promise<HyperchartRunInfo> {
	...
	return completionScenario.runtimeRun(runtime.records, {
		runId: `completion:${snapshot}`, status: { state: snapshot === "consumed" ? "complete" : snapshot === "notify-failed" ? "failed" : "running" },
		cwd: "/storybook/completion", createdAt: runtime.records[0]?.timestamp ?? Date.now(),
