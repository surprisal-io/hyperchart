...
import {
	actorMapLocalRun, actorNamedReplyRun, actorPendingCallRun, actorPoolCrowdedRun,
	actorSelfSendRun, actorSendVoidRun, allActorPoolRuns, allActorRuns,
...
export type AtlasCase = { title: string; run: HyperchartRunInfo; stateId: string; status: HyperchartStateInfo["status"] };
...
const runtimeRuns = [
	...
	...allActorPoolRuns,
	actorPoolCrowdedRun, actorSelfSendRun, actorSendVoidRun,
	actorNamedReplyRun, actorPendingCallRun, actorMapLocalRun,
	...allRunStripRuns,
];
...
export function atlasCases(kind: HyperchartStateType): AtlasCase[] {
	...
	for (const spec of inspectorPanelSpecs) {
		if (spec.runtime.selectedStateId === null) continue;
		...
		if (state?.type === kind) cases.push(fromRun(scenario.run, state, spec.title));
	}
	...
	for (const run of runtimeRuns) {
		for (const state of run.states) {
			if (state.type !== kind || statuses.has(state.status)) continue;
			cases.push(fromRun(run, state, `${kind} · ${state.status}`));
			statuses.add(state.status);
		}
	}
	if (kind === "region" || kind === "compound" || kind === "gate") {
		const title = kind === "region" ? "Parallel branch scope" : kind === "compound" ? "Compound scope" : "Pending host gate";
		const spec = inspectorPanelSpecs.find((candidate) => candidate.title === title);
		if (spec === undefined || spec.runtime.selectedStateId === null) throw new Error(`Missing ${kind} fixture`);
		...
		if (state?.type !== kind || state.status !== "pending") throw new Error(`Expected pending ${kind} fixture`);
	...
	if (kind === "region") {
		...
		if (spec === undefined) throw new Error("Missing completed parallel fixture");
		...
		if (run === undefined || state === undefined) throw new Error("Expected completed parallel region fixture");
	...
	if (kind === "reply") {
		const state = actorNamedReplyRun.states.find((candidate) => candidate.type === "reply" && candidate.status === "done");
	...
	if (cases.length === 0) throw new Error(`No adapter-derived Card Atlas case for ${kind}`);
	return cases;
}
