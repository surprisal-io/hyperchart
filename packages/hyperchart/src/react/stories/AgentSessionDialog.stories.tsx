...
const sessionScenario = storyScenario(
	chart({
		kind: "chart",
...
const completedRecords = [...activeRecords, storyComplete(sessionScenario.ast, "draft", "DONE", 3, 1_700_000_060_000)];
...
const liveProgress: HyperchartRuntimeSessionProgressFile = {
	...
	sessions: {
		live: {
			...
			tokenCount: 8_412,
			currentReasoning:
				"I need to preserve the user's scroll position while appending new deltas. I’ll only stick to the bottom when the viewport is already near it.",
			...
			messages: [
				{ id: "u1", role: "user", text: "Add a live session view to agent cards.", timestamp: draftInvokedAt },
				{
					id: "r1",
					role: "reasoning",
					text: "The card needs a stable action key and a compact session snapshot. The browser should never read the session file directly.",
					timestamp: 1_700_000_008_000,
				},
				{
					id: "a1",
					role: "assistant",
					text: "I’ll inspect the existing card and modal patterns, then wire the runtime data through the host model.",
				...
				},
				{
					id: "a2",
					role: "assistant",
					text: "The inspector already has portal, theme, and focus primitives. I’m reusing them for a compact transcript and steering composer.",
...
const completedSession = projectedSession(completedRecords, completedProgress, {
	state: "complete",
	updatedAt: 1_700_000_060_000,
});
		...
		collapsed: {
			...
			messages: [
				...
				},
				{
					...
					toolInput: '{ "path": "AgentInfoCard.tsx" }',
					toolOutput:
						"export function AgentInfoCard({ state }: AgentInfoCardProps) {\n  return <div>{state.agent}</div>;\n}",
					toolStatus: "completed",
				},
				{
					id: "tool-long",
...
const collapsedSession = projectedSession(completedRecords, collapsedProgress, {
	state: "complete",
	updatedAt: 1_700_000_060_000,
});
...
const meta = {
	...
	parameters: {
		...
		docs: {
			description: {
				component:
					"Agent sessions projected from a normalized chart, durable visits, and sessions/progress.json input.",
			},
		},
	},
