...
const progress: HyperchartRuntimeSessionProgressFile = {
	...
	sessions: {
		visit1: {
			actionUid: researchUid,
			visit: 1,
			actionKey: `${researchUid.chart}:${researchUid.state}:${researchUid.action}`,
			status: "completed",
			startedAt: 1_700_000_010_000,
			completedAt: 1_700_000_020_000,
			model: researchAction.model,
			thinking: researchAction.thinking,
			turnCount: 2,
			toolCount: 3,
		...
		visit2: {
			...
			messages: [
				...
				{ id: "a1", role: "assistant", text: "Current visit is still running." },
			],
		},
	},
};
...
const run = scenario.runtimeRun(records, {
	runId: "runtime-section:research",
...
const runWithoutSession = scenario.runtimeRun(records, {
	runId: "runtime-section:no-session",
	status: { state: "running", updatedAt: 1_700_000_040_000 },
	cwd: "/workspace",
	createdAt: 1_700_000_000_000,
	updatedAt: 1_700_000_040_000,
...
const secondProgress: HyperchartRuntimeSessionProgressFile = {
	updatedAt: 1_700_000_070_000,
	sessions: {
		second: {
			actionUid: secondUid,
			visit: 1,
			actionKey: `${secondUid.chart}:${secondUid.state}:${secondUid.action}`,
			status: "running",
...
const secondRun = scenario.runtimeRun(secondRecords, {
	runId: "runtime-section:second",
	status: { state: "running", updatedAt: 1_700_000_070_000 },
	sessionProgress: secondProgress,
	cwd: "/workspace",
});
	...
	return (
		<div className="space-y-2">
			<div className="flex flex-wrap gap-2">
				...
				<button type="button" onClick={() => setPhase("second")}>
...
const meta = {
	title: "Hyperchart/Inspector/Runtime Section",
	id: "hyperchart-components-runtime-section",
	component: RuntimeSection,
	parameters: {
		layout: "centered",
		docs: {
			description: {
				component: "Runtime facts and sessions projected from durable visits plus sessions/progress.json input.",
			},
		},
	},
	args: { state, onSteerSession: fn() },
	decorators: [
		(Story) => (
