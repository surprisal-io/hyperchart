...
function useCapturedRun(key: Snapshot | "batch-result") {
	const [run, setRun] = useState<HyperchartRunInfo>();
	const [error, setError] = useState<string>();
	useEffect(() => {
		let active = true;
		const capture = key === "batch-result" ? captureCallBatchResultStory() : captureCompletionStory(key);
		void capture.then(
			(value) => {
				if (active) {
					setRun(value);
...
function CompletionRuntimeCard({ snapshot, stateId, title }: { snapshot: Snapshot; stateId: string; title: string }) {
	const { run, error } = useCapturedRun(snapshot);
	if (error !== undefined) {
...
export function CompletionRuntimePair() {
	const { run, error } = useCapturedRun("consumed");
	if (error !== undefined) {
		return <div role="alert">{error}</div>;
	}
	if (run === undefined) {
		return <div role="status">Executing and replay-validating completion graph…</div>;
	}
	return (
		<GraphTile
			title="completion notify → waitFor · consumed"
			run={run}
...
function ActorCompletionCase({ snapshot, stateId }: { snapshot: Snapshot; stateId: "@worker.publish" | "wait" }) {
	...
	if (run === undefined) {
		return <div role="status">Executing and replay-validating completion fixture…</div>;
	}
	const ast = completionScenario.ast;
	const label = stateId === "wait" ? "Wait For" : "Notify · @worker.publish";
	const phase =
		snapshot === "notify-in-flight"
			? "sending"
			: snapshot === "notification-retained"
				? stateId === "wait"
					? "pending"
					: "published"
				: snapshot === "notify-failed"
					? "failed"
					: snapshot === "waiting"
						? "waiting"
						: "received";
	return (
		<InspectorPanelTile
			variant="panel"
			title={`${label} · ${phase}`}
			description="Authored event contract and executed, replay-validated visit."
			run={run}
...
export function CapturedActorDialog({ kind }: { kind: "completion" | "batch" }) {
	const { run, error } = useCapturedRun(kind === "completion" ? "consumed" : "batch-result");
	if (error !== undefined) {
		return <div role="alert">{error}</div>;
	}
	if (run === undefined) {
		return <div role="status">Executing and replay-validating actor fixture…</div>;
	}
	return <InteractiveInspector runs={[run]} selectedRunId={run.runId} onClose={() => undefined} />;
}

function ActorBatchResultCase() {
	const { run, error } = useCapturedRun("batch-result");
	if (error !== undefined) {
		return <div role="alert">{error}</div>;
	}
	if (run === undefined) {
		return <div role="status">Executing and replay-validating batch fixture…</div>;
	}
		...
		<div className="grid gap-4 xl:grid-cols-2">
			{(["batch", "use"] as const).map((stateId) => (
				<InspectorPanelTile
					key={stateId}
					variant="panel"
					title={stateId === "batch" ? "callBatch" : "result(batch) consumer"}
					description="Executed durable visits and adapter-derived state details."
					run={run}
					selectedStateId={stateId}
					definitionSource={hyperchartSource(ast, stateId)}
					runtimeSources={[{ title: "Definition", code: hyperchartSource(ast, stateId), language: "typescript" }]}
...
export function ActorCompletionCases() {
	return (
		<>
			<BoardSection
				title="Notify · @worker.publish"
				description="Actual sending, published event with payload, and failed delivery."
			>
			...
			<BoardSection
				title="Wait For · root wait"
				description="Event signature, pending and active wait, then received payload."
			>
				<div className="space-y-5">
...
export function ActorRuntimeCases() {
	return (
		<BoardSection
			title="Ordered callBatch result"
			description="Real execution-loop facts, replay validation, and production host projections."
		>
