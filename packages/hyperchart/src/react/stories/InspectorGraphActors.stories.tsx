...
export const ActorNodesAndEdges: Story = {
	render: () => (
		<BoardPage
			title="Explicit actor graph"
			description="Adapter-derived singleton actors and bounded pools: one endpoint node plus one canonical worker workflow scope."
		>
			<div className="grid gap-4 xl:grid-cols-2">
				...
				<GraphTile
					title="self-send · shared pool endpoint"
					run={actorSelfSendRun}
					height="h-[620px]"
					className="xl:col-span-2"
				/>
				<GraphTile
					title="nested structured drain · SEND-only"
					run={actorDrainingRun}
					visibleStateIds={drainingRootStateIds}
					height="h-[480px]"
				/>
			</div>
		</BoardPage>
