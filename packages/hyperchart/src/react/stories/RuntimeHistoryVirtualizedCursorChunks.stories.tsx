...
} from "../../inspect/history_mapping.js";
import type {
...
class SemanticStoryRuntime implements Runtime {
	...
	constructor(
		...
		private readonly targetType: DurableLogRecord["type"],
				...
				const records = effect.records.map(
					(draft): DurableLogRecord =>
						({
							...
							branchId: "main",
							timestamp: 1_700_100_000_000 + this.seqId,
						}) as DurableLogRecord,
				);
...
async function captureSemantic(
	cst: ChartCst,
	args: Readonly<Record<string, unknown>>,
	targetType: DurableLogRecord["type"],
	count: number,
) {
...
async function captureRuntime(visits: number): Promise<Fixture> {
	const cst = chart({
		...
		states: {
			work: {
				kind: "state",
				action: script("node", ["-e", "process.exit(0)"]),
				transitions: { DONE: "work", STOP: "done" },
			},
			done: final(),
		},
	});
	...
	const run = hyperchartRunFromRuntime(
		inspectChartAst(runtime.ast, { chartPath: "storybook:runtime-history" }),
		runtime.ast,
		runtime.records,
	);
	...
	return {
		...
		state: withoutElapsedHistory(state),
		allStates: run.states.map(withoutElapsedHistory),
	};
}
...
function withoutElapsedHistory(state: HyperchartStateInfo): HyperchartStateInfo {
	...
	const mapConfig =
		state.mapConfig === undefined ? undefined : (({ visitHistory: _mapHistory, ...value }) => value)(state.mapConfig);
	const actorOccurrence =
		state.actorOccurrence === undefined
			...
			: (({
					...
					mailboxInstances: _mailboxInstances,
					...value
				}) => ({ ...value, mailboxInstances: [] }))(state.actorOccurrence);
	return {
		...summary,
...
function fixtureFromRuntime(
	captured: { ast: ChartAst; records: readonly DurableLogRecord[] },
	rows: readonly StoryRow[],
	state: (states: readonly HyperchartStateInfo[]) => HyperchartStateInfo | undefined,
): Fixture {
	const run = hyperchartRunFromRuntime(
		inspectChartAst(captured.ast, { chartPath: `storybook:${captured.ast.id}` }),
		captured.ast,
		captured.records,
	);
	...
	return {
		...
		state: withoutElapsedHistory(selected),
		allStates: run.states.map(withoutElapsedHistory),
	};
}
...
async function captureSemanticRows(kind: "map" | "generations" | "messages", count: number): Promise<Fixture> {
		...
		const cst = chart({
			...
			states: {
				items: mapState({
					...
					initial: "work",
					onDone: "items",
					states: { work: { kind: "state", action: script("true"), transitions: { DONE: "done" } }, done: final() },
				}),
			},
		});
		const captured = await captureSemantic(cst, { items: { only: 1 } }, "spawned", count);
		const rows = captured.records
			.filter((record): record is Extract<DurableLogRecord, { type: "spawned" }> => record.type === "spawned")
			.map((spawn, index): StoryRow => {
				const value = mapVisitHistoryItemToHost({
					kind: "map-visit",
					mapPath: spawn.path,
					seqId: spawn.seqId,
					visit: index + 1,
	...
	const Work = protocol({
		WORK: message({ input: z.object({ index: z.number() }).strict(), reply: z.object({ ok: z.boolean() }).strict() }),
	});
	const Worker = actor({
		input: z.object({}).strict(),
		protocol: Work,
		initial: "idle",
		states: { idle: receive({ on: { WORK: "reply" } }), reply: reply({ target: "idle", output: { ok: true } }) },
	});
	const worker = Worker({});
	if (kind === "generations") {
		const cst = chart({
			kind: "chart",
			id: "history-generations",
			initial: "phase",
			states: {
				phase: compound({
					actors: { worker },
					initial: "dispatch",
					onDone: "pause",
					states: { dispatch: send({ to: worker, event: "WORK", input: { index: 1 }, target: "done" }), done: final() },
				}),
				pause: { kind: "state", action: script("true"), transitions: { DONE: "phase" } },
			},
		});
		const captured = await captureSemantic(cst, {}, "actor_created", count);
		const rows = captured.records
			.filter(
				(record): record is Extract<DurableLogRecord, { type: "actor_created" }> => record.type === "actor_created",
			)
			.map((created): StoryRow => {
				const value = actorGenerationHistoryItemToHost({
					kind: "actor-generation",
					logicalOccurrence: created.occurrence.replace(/~\d+$/, ""),
					seqId: created.seqId,
					created,
					records: [created],
				} satisfies ActorGenerationHistoryItem);
		...
		return fixtureFromRuntime(captured, rows, (states) =>
			states.find(
				(state) =>
					state.actorOccurrence?.logicalPath === logicalOccurrence ||
					state.actorInternal?.logicalOccurrencePath === logicalOccurrence,
			),
		);
	}
	const cst = chart({
		...
		actors: { worker },
		initial: "send",
		states: { send: send({ to: worker, event: "WORK", input: { index: 1 }, target: "send" }) },
	});
	const captured = await captureSemantic(cst, {}, "actor_messages_enqueued", count);
	const historyItems = captured.records
		.filter(
		...
		.map(
			(enqueued) =>
				({
					...
					seqId: enqueued.seqId,
					enqueued,
					records: [enqueued],
				}) satisfies ActorMessageHistoryItem,
		);
	const rows = actorMessageHistoryItemsToHost(historyItems, captured.ast, captured.records)
		.map((value): StoryRow => ({ kind: "messages", id: `${value.occurrencePath}:${value.enqueueSeqId}`, value }))
		.reverse();
	const occurrence = rows.find((row) => row.kind === "messages")?.value.occurrencePath;
	return fixtureFromRuntime(captured, rows, (states) =>
		states.find(
			(state) =>
				state.actorOccurrence?.occurrencePath === occurrence || state.actorInternal?.occurrencePath === occurrence,
		),
	);
}
	...
	switch (seed.kind) {
		case "visit": {
			const value = {
				...
				visit: ordinal,
				invokeSeqId: seqId,
				startedAt: 1_700_100_000_000 + seqId,
				...(seed.value.endedAt === undefined ? {} : { endedAt: 1_700_100_000_001 + seqId }),
			};
		...
		case "generation": {
			const value = {
				...seed.value,
				occurrencePath: `${seed.value.logicalOccurrence}~${ordinal}`,
				generation: ordinal,
				createdSeqId: seqId,
				createdAt: 1_700_100_000_000 + seqId,
			};
			return { kind: "generation", id: `${value.occurrencePath}:${value.createdSeqId}`, value };
		}
		case "messages": {
			const value = {
				...
				messages: seed.value.messages.map((message) => ({
					...message,
					messageId: `${message.messageId}:${ordinal}`,
					producerVisit: String(ordinal),
				})),
			};
...
function stateRows(items: readonly StateVisitHistoryItem[]): readonly StoryRow[] {
	return items.map((item): StoryRow => {
		const value = stateVisitHistoryItemToHost(item);
		return { kind: "visit", id: String(value.invokeSeqId), value };
	});
}
...
function storyChunk(fixture: Fixture, cursor?: HistoryCursor): HistoryChunk<StoryRow> {
	...
	return {
		snapshot,
		items,
		...(end < fixture.rowCount ? { older: `older:${end - 1}` } : {}),
		...(start > 0 ? { newer: `newer:${start}` } : {}),
	};
}

type Scenario = "state-visits" | "map" | "generations" | "messages";

function ProductionHistoryStory({
	fixture,
	scenario,
	source,
}: {
	fixture: Fixture;
	scenario: Scenario;
	source: { load(cursor?: HistoryCursor): Promise<HistoryChunk<StoryRow>> };
}) {
		...
		() => ({
			...
			readStateVisits: async (input) => {
				...
				return {
					...
					items: chunk.items.flatMap((row) => (row.kind === "visit" ? [row.value] : [])),
				};
			},
			readMapVisits: async (input) => {
				const chunk = await source.load(input.cursor);
				return {
					...chunk,
					snapshot: input.snapshot,
					items: chunk.items.flatMap((row) => (row.kind === "map" ? [row.value] : [])),
				};
			},
			readActorGenerations: async (input) => {
				const chunk = await source.load(input.cursor);
				return {
					...chunk,
					snapshot: input.snapshot,
					items: chunk.items.flatMap((row) => (row.kind === "generation" ? [row.value] : [])),
				};
			},
			readActorMessages: async (input) => {
				const chunk = await source.load(input.cursor);
				return {
					...chunk,
					snapshot: input.snapshot,
					items: chunk.items.flatMap((row) => (row.kind === "messages" ? [row.value] : [])),
				};
			},
	...
	return (
		<div
			...
			data-history-scenario={scenario}
			data-selected-history-subject={fixture.state.runtimeStatePath ?? fixture.state.id}
		>
			<div className="mx-auto max-w-3xl rounded-xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
				<div className="mb-3">
					<h2 className="text-sm font-semibold">Runtime History — Virtualized Cursor Chunks</h2>
					<p className="text-xs text-[var(--text-muted)]">
						{scenario} · HyperchartInspectorDataSource → production RuntimeSection
					</p>
				</div>
				<RuntimeSection
					state={fixture.state}
					allStates={[...fixture.allStates]}
					history={{ runId: "storybook-runtime-history", snapshot, dataSource }}
				/>
			</div>
		</div>
	);
...
function RuntimeHistoryStory({ scenario }: { scenario: Scenario }) {
	...
	useEffect(() => {
		let current = true;
		const pending =
			scenario === "map" || scenario === "generations" || scenario === "messages"
	...
	const source = useMemo(
		() => ({
			load: async (cursor?: HistoryCursor) =>
				fixture === undefined ? { snapshot, items: [] } : storyChunk(fixture, cursor),
		}),
		[fixture],
	);
...
async function storyHistoryList(canvasElement: HTMLElement) {
	...
	switch (scenario) {
		case "map":
			disclosure = "Load map launch history";
			break;
		case "generations":
			disclosure = "Load actor generations";
			break;
		case "messages":
			disclosure = "Load actor message history";
			break;
		default:
			break;
	}
