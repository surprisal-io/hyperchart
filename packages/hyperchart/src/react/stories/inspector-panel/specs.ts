...
function pushFailure(builder: StoryLogBuilder, ast: ChartAst, statePath: StatePath, error: unknown): void {
	const actionUid = pushInvoke(builder, ast, statePath);
	...
	const target = { kind: "action" as const, actionUid, phase: "running" as const };
	const requestId = `storybook:failure-cancel:${statePath}`;
	builder.records.push({ type: "cancellation", kind: "requested", requestId, target, parentId: builder.seq, seqId: ++builder.seq, timestamp: storyTimestamp(builder.seq) });
	builder.records.push({ type: "cancellation", kind: "acknowledged", requestId, target, parentId: builder.seq, seqId: ++builder.seq, timestamp: storyTimestamp(builder.seq) });
}
