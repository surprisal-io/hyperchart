...
export function HyperchartStateGraphNode({ data, selected }: NodeProps<StateNode>) {
	...
	const actorMessage =
		displayState.type === "send" ||
		displayState.type === "sendBatch" ||
		displayState.type === "call" ||
		displayState.type === "callBatch"
	...
	const displayedStatus =
		displayState.type === "actor-declaration" && state.actorOccurrence === undefined ? "definition-only" : state.status;
	...
	return (
		<div className="relative" style={{ width: size.width, height: size.height }}>
			{isActorPool && (
				<>
					<div
						className="pointer-events-none absolute inset-0 translate-x-2 translate-y-2 rounded-xl border border-amber-500/15 bg-[var(--bg-tertiary)] shadow-md"
						aria-hidden="true"
					/>
					<div
						className="pointer-events-none absolute inset-0 translate-x-1 translate-y-1 rounded-xl border border-amber-500/20 bg-[var(--bg-secondary)] shadow-md"
						aria-hidden="true"
					/>
				</>
			)}
			<div
				className={`absolute inset-0 z-10 overflow-hidden rounded-xl border bg-[var(--bg-secondary)] shadow-lg ${outlineClass}`}
			>
				<div
				...
				<div className="flex h-full min-w-0 flex-col px-3 pb-5 pt-3">
					<div className="flex min-w-0 items-center gap-1.5">
						...
						>
							<span className={`h-1.5 w-1.5 rounded-full ${hyperchartStatusDotClass(state.status)}`} />{" "}
							{displayedStatus}
						</span>
					</div>
						...
						<div className="mt-2 flex min-w-0 items-center gap-2 text-[10px]">
							{actorMessage !== undefined ? (
								<div className="flex min-w-0 items-center font-mono text-[10px] leading-none text-[var(--text-secondary)]">
									<EnvelopeIcon
										className="mr-1 h-2.5 w-2.5 shrink-0 text-[var(--text-muted)]"
										aria-label="Event"
										title="Event"
									/>
									<span className="truncate text-[var(--text-primary)]" title={actorMessage.event}>
										{actorMessage.event}
									</span>
									<span className="mx-1.5 text-[var(--text-muted)]" aria-hidden="true">
										→
									</span>
									<span className="truncate" title={actorMessage.to}>
										{actorMessage.to}
									</span>
								</div>
							) : !hasStructuredPreview ? (
								<div
									className="min-w-0 truncate font-mono text-[var(--text-secondary)]"
									title={mechanism ?? kind.label}
								>
