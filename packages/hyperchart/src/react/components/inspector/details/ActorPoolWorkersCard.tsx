	...
	return (
		<div className="grid min-w-0 gap-1">
			{occurrence.workers.map((worker) => {
				...
				return (
					<div
						key={worker.occurrencePath}
						className={`min-w-0 overflow-hidden rounded border ${selected ? "border-cyan-500/40 bg-cyan-500/5" : "border-[var(--border-secondary)] bg-[var(--bg-secondary)]"}`}
					>
						<button
							type="button"
						...
						>
							<ChevronRightIcon
								className={`h-3.5 w-3.5 shrink-0 text-[var(--text-muted)] transition-transform ${selected ? "rotate-90" : ""}`}
							/>
							<code className="shrink-0 text-[var(--text-primary)]">$worker-{worker.index}</code>
							<span
								title="Current state"
								className="inline-flex min-w-0 items-center gap-1 rounded border border-cyan-500/25 bg-cyan-500/5 px-1.5 py-0.5 text-[var(--hc-cyan-text)]"
							>
