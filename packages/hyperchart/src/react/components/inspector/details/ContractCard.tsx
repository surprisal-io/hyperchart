			...
			{hasArtifacts && (
				<div className="space-y-2">
					...
					{state.artifacts?.map((artifact) => (
						<div
							id={artifactContractElementId(state.id, artifact.name)}
							key={artifact.name}
							className={`rounded-lg border bg-purple-500/10 p-2 ${highlightedArtifactName === artifact.name ? "border-purple-400 ring-2 ring-purple-500/30" : "border-purple-500/20"}`}
						>
							<div className="flex flex-wrap items-center gap-1.5">
