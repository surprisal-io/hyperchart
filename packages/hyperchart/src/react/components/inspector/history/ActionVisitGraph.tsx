...
type MapGroupNode = Node<Readonly<{
...
type ExecutionBlock =
	...
	| Readonly<{ kind: "scope"; groupId: string; groupType: "map" | "parallel" | "actor-occurrence"; cycle: number; entries: VisitEntry[]; width: number; height: number }>;
...
export function ActionVisitGraph({
	...
	return (
		<section data-hyperchart-execution className="relative min-h-0 flex-1 bg-[var(--bg-primary)]" aria-label="Hierarchical execution graph">
			<div className="absolute right-3 top-3 z-10 flex items-center gap-1.5">
				{history.hasOlder && (
					<button type="button" className="min-h-8 rounded-lg border border-[var(--border-secondary)] bg-[var(--bg-secondary)] px-3 text-[10px] text-[var(--hc-cyan-text)] shadow-sm" disabled={history.older.loading} onClick={() => void history.loadOlder()}>
						{history.older.loading ? "Loading…" : "Older"}
					</button>
				)}
				{history.hasNewer && (
					<button type="button" className="min-h-8 rounded-lg border border-[var(--border-secondary)] bg-[var(--bg-secondary)] px-3 text-[10px] text-[var(--hc-cyan-text)] shadow-sm" disabled={history.newer.loading} onClick={() => void history.loadNewer()}>
			...
			{history.initialError !== undefined ? (
				...
				<div className="grid h-full place-items-center p-8 text-sm text-[var(--text-muted)]">{history.loading ? "Loading execution…" : "No action visits in this window."}</div>
			) : (
				<ReactFlow
					...
					onNodeClick={(_, node) => {
						...
						if (node.type === "mapVisitGroup") onSelectState(String(node.data.stateId), node.id, typeof node.data.targetSeqId === "number" ? node.data.targetSeqId : undefined);
					}}
				>
...
export function executionGraph(run: HyperchartRunInfo, rows: readonly ActionVisitRow[]): { nodes: ExecutionNode[]; edges: Edge[] } {
	...
	for (const block of blocks) {
		...
		if (block.kind === "action") {
			...
			const prefix = block.groupType === "map" ? "map-visit" : block.groupType === "parallel" ? "parallel-visit" : "actor-visit";
			...
			const targetSeqId = block.groupType === "map"
			...
			} else lanes.forEach((lane, laneIndex) => {
...
function executionEntries(run: HyperchartRunInfo, rows: readonly ActionVisitRow[]): VisitEntry[] {
	...
	return rows.map((row) => {
		...
		const workerScopeId = template === undefined ? workerScopeFromRuntimePath(row.statePath, mapIds) : stateScopeParentId(template);
		...
		const authoredScope = mapId === undefined && template !== undefined ? authoredExecutionScopeForState(template, statesById) : undefined;
...
function authoredExecutionScopeForState(
	...
	while (parentId !== undefined && !seen.has(parentId)) {
		...
		if (parent.type === "actor-occurrence") return { groupId: parent.id, groupType: "actor-occurrence", laneScopeId: parent.id };
		child = parent;
...
function executionBlocks(entries: readonly VisitEntry[]): ExecutionBlock[] {
	const provisional: Array<{ kind: "action"; entry: VisitEntry } | { kind: "scope"; groupId: string; groupType: "map" | "parallel" | "actor-occurrence"; entries: VisitEntry[] }> = [];
	...
	return provisional.map((block) => {
		...
		if (block.groupType === "actor-occurrence") {
			...
			const height = MAP_HEADER + block.entries.length * ACTION_HEIGHT + Math.max(0, block.entries.length - 1) * ACTION_GAP_Y + MAP_PADDING_BOTTOM;
		...
		const height = MAP_HEADER + Math.max(...lanes.map((lane) => workerHeightFor(lane.entries.length))) + MAP_PADDING_BOTTOM;
		return { ...block, cycle, width, height };
	});
}
...
function workerHeightFor(actionCount: number): number {
	return WORKER_HEADER + actionCount * ACTION_HEIGHT + Math.max(0, actionCount - 1) * ACTION_GAP_Y + WORKER_PADDING_BOTTOM;
}
...
function MapVisitGroupNode({ data, selected }: NodeProps<MapGroupNode>) {
	...
	return (
		<div className={`h-full w-full cursor-pointer rounded-xl border-2 bg-cyan-500/[0.04] shadow-sm transition-[border-color,box-shadow] ${selected ? "border-blue-400 ring-2 ring-blue-500/30" : "border-cyan-500/35 hover:border-cyan-400/70"}`}>
			...
			<div className="flex h-[54px] min-w-0 items-center gap-1.5 border-b border-cyan-500/20 px-4">
				...
				<div className="min-w-0 flex-1 truncate font-mono text-xs font-semibold text-[var(--text-primary)]" title={data.stateId}>{stateDisplayName(data.state)}</div>
			</div>
...
function WorkerLaneNode({ data }: NodeProps<WorkerLaneNode>) {
	return (
		<div className="h-full w-full rounded-lg border border-[var(--border-secondary)] bg-[var(--bg-secondary)]/70">
			<div className="flex h-[38px] items-center border-b border-[var(--border-primary)] px-3 font-mono text-[10px] font-medium text-[var(--text-secondary)]">{data.label}</div>
		</div>
	);
}
