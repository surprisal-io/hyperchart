...
export function VirtualizedHistoryList<T>({
	...
	if (usePlainLayout) {
		return (
			<div
				...
				data-testid="virtualized-history"
				data-history-layout="plain"
	...
	return (
		<div
			className="grid gap-1.5"
			data-testid="virtualized-history"
			data-history-layout="virtual"
			data-retained-items={items.length}
		>
			{history.newer.error !== undefined && (
				<HistoryError edge="newer" error={history.newer.error} onRetry={history.loadNewer} />
			)}
			...
			<div ref={parentRef} className={`overflow-auto overscroll-contain ${className}`} onScroll={prefetchAtScrollEdge}>
				<div className="w-full">
					{beforeSize > 0 && (
						<div aria-hidden="true" data-history-spacer="before" style={{ height: `${beforeSize}px` }} />
					)}
			...
			{history.older.error !== undefined && (
				<HistoryError edge="older" error={history.older.error} onRetry={history.loadOlder} />
			)}
		</div>
	);
}
...
function HistoryError({ edge, error, onRetry }: { edge: string; error: string; onRetry: () => void | Promise<void> }) {
	return (
		<div className="flex items-center justify-between gap-2 rounded border border-red-500/30 bg-red-500/5 p-2 text-[10px] text-[var(--danger)]">
			<span>
				{edge} load failed: {error}
			</span>
			<button type="button" className="text-[var(--hc-cyan-text)]" onClick={() => void onRetry()}>
				Retry
			</button>
		</div>
	);
