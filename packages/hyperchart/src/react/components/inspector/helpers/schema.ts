...
export function schemaToTs(value: unknown, depth = 0): string {
	...
	if (enumValues?.length) {
		return enumValues.map(literalTs).join(" | ");
	}
	if ("const" in schema) {
		return literalTs(schema.const);
	}
	...
	if (allOf) {
		return allOf.replace(/ \| /g, " & ");
	}
	const type = schema.type;
	if (Array.isArray(type)) {
		return type.map((item) => schemaToTs({ ...schema, type: item }, depth)).join(" | ");
	}
	if (type === "string") {
		return "string";
	}
	...
	if (type === "object" || schema.properties || schema.additionalProperties) {
		return schemaObjectToTs(schema, depth);
	}
	return "unknown";
}
		...
		if (additional !== false) {
			...
			const additionalType =
				additionalSchema && Object.keys(additionalSchema).length > 0
					...
					: "unknown";
			if (entries.length === 0) {
				return [{ text: `Record<string, ${additionalType}>` }];
			}
			lines.push({ text: `${inner}[key: string]: ${additionalType};` });
		}
		if (lines.length === 1) {
			return [{ text: "Record<string, unknown>" }];
		}
		lines.push({ text: `${pad}}` });
		return lines;
	}
...
export function typeAliasLines(
	...
	if (!schema.schema) {
		return schema.schemaName ? [{ text: `type ${alias} = ${schema.schemaName};` }] : undefined;
	}
	...
	if (body.length === 1) {
		return [{ ...body[0], text: `type ${alias} = ${body[0]?.text ?? "unknown"};` }];
	}
	return [
...
export function schemaTypeText(schema: HyperchartStateInfo["replySchema"]): string {
	...
	if (schema.schema) {
		return schemaToTs(schema.schema, 0);
	}
