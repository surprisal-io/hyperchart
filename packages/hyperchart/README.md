...
Runs are addressed only by `runId`. Host storage config declares backend, root and `run-id` or `sha256` layout; existing literal framework and hashed AutoDiscovery layouts remain unchanged. See the [canonical runtime identity contract](../../docs/api/runtime.md#run-identity-and-storage-scope). Owning schedulers distinguish retryable branch seals from unrecoverable `BranchDrainError` / `BranchAdmissionError`; see the [runner lifecycle contract](../../docs/api/runtime.md).
