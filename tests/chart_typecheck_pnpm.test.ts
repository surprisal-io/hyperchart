...
it("chart_typecheck resolves its own package when TypeScript occupies a separate pnpm virtual-store entry", async () => {
	...
	try {
		...
		writeFileSync(join(pkg, "package.json"), JSON.stringify({
			name: "@surprisal/hyperchart", type: "module",
		...
		writeFileSync(join(pkg, "src/runtime/generic/chart_typecheck.js"), ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 } }).outputText);
		...
		writeFileSync(chart, 'import { marker } from "@surprisal/hyperchart";\nconst checked: "self-package" = marker;\nexport default checked;\n');
