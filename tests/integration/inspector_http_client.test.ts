...
describe("browser inspector history transport", () => {
	it("decodes explicit absence for missing cursors and visit sessions", async () => {
		...
		await expect(
			source.cursorAt({ runId: "run", snapshot, subject: { kind: "records" }, seqId: 9 }),
		).resolves.toBeUndefined();
		await expect(
			source.readVisitSession({ runId: "run", snapshot: { branchId: "main", headSeqId: 9 }, invokeSeqId: 9 }),
		).resolves.toBeUndefined();
	...
	it("rejects malformed success envelopes", async () => {
		...
		await expect(
			browserHistoryDataSource("token").readVisitSession({
				runId: "run",
				snapshot: { branchId: "main", headSeqId: 9 },
				invokeSeqId: 9,
			}),
		).rejects.toThrow(/History request failed/);
	});
});
