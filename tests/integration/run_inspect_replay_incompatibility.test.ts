...
import {
	...
	hyperchartRunOverviewFromRunId,
	readBranchExecutionOverview,
} from "../../packages/hyperchart/src/inspect/run_inspect.js";
...
async function fixture() {
	const root = mkdtempSync(join(tmpdir(), "hyperchart-replay-recovery-"));
	roots.push(root);
	const runId = "run";
	...
	await store.writeRunMeta({
		runId,
		chartPath: join(root, "chart.ts"),
		workDir: root,
		chartId: originalReplayScenario.ast.id,
	...
	const captured = await captureReplayIncompatibleHistory((drafts) => store.appendDrafts(drafts), {
		"candidate.txt": candidatePin,
	});
...
function bytes(root: string): Record<string, string> {
	return Object.fromEntries(
		readdirSync(root).flatMap((name): [string, string][] => {
			const path = join(root, name);
			return statSync(path).isDirectory()
				? Object.entries(bytes(path)).map(([child, value]) => [`${name}/${child}`, value])
	...
	try {
		...
		expect(
			f.records
				.filter(
					(record) =>
						record.type === "state_action" && record.kind === "validated" && record.actionUid.state === "experiment",
				)
				.map((record) => record.type === "state_action" && record.kind === "validated" && record.outcome),
		).toEqual([{ ok: false, reason: "record lab notes and retry" }, true]);
		await expect(
			BranchExecution.restore({
				ast,
				branchId: "historical",
				store: f.store,
				snapshot: f.snapshot,
		...
		).rejects.toThrow("Invalid action invoke");
		await expect(readBranchExecutionOverview(ast, "historical", f.store, f.snapshot)).rejects.toThrow(
			"Invalid action invoke",
		);
		expect(() => hyperchartRunFromRuntime(changedReplayScenario.inspect, ast, f.records)).toThrow(
			"Invalid action invoke",
		);
		const overview = await withRunStorage(f.storage, () =>
			hyperchartRunOverviewFromRunId(f.runId, { ast, snapshot: f.snapshot }),
		);
		expect(overview.snapshot).toEqual(f.snapshot);
		expect(overview.run).toMatchObject({
			mode: "static",
			status: "blocked",
			replayIncompatibility: { seqId: f.broken.seqId, stateId: "experiment" },
		});
		expect(overview.run.issues?.[0]).toMatchObject({
			kind: "replay_warning",
			source: "durable_log",
			seqId: f.broken.seqId,
		});
		expect(overview.run.states).toEqual(changedReplayScenario.staticRun().states);
		expect(overview.run.finalOutput).toBeUndefined();
		expect(renderToStaticMarkup(createElement(RunOverview, { run: overview.run }))).toContain(
		...
		const lastCompletion = [...f.records]
			.reverse()
			.find(
				(record) => record.type === "state_action" && record.kind === "complete" && record.actionUid.state === "after",
			)!;
		const readTranscript = vi.fn(async ({ sessionId }: { sessionId: string }) =>
			sessionId === last.sessionId
				? [
						{
							id: "after",
							role: "assistant" as const,
							text: "recorded after incompatibility",
							timestamp: lastCompletion.timestamp,
						},
						{ id: "future", role: "assistant" as const, text: "not in snapshot", timestamp: Number.MAX_SAFE_INTEGER },
					]
				: [],
		);
		const source = await withRunStorage(f.storage, () =>
			createRunInspectorDataSource(f.runId, { ast, readTranscript }),
		);
		const returned = [];
		...
		expect(returned.find((record) => record.seqId === last.seqId)?.actionVisit).toMatchObject({
			status: "unknown",
			originBranchId: "main",
			replayWarning: expect.stringContaining("Recorded facts only"),
		});
		...
		expect(afterVisits.items[0]).toMatchObject({
			invokeSeqId: last.seqId,
			status: "unknown",
			originBranchId: "main",
			inputs: { topic: "durable evidence" },
			invocation: { task: "Record {{…}}" },
		});
		const experiment = await source.readStateVisits({ runId: f.runId, snapshot: f.snapshot, stateId: "experiment" });
		expect(experiment.items[0]).toMatchObject({
			status: "done",
			validationAttempts: 2,
			replayWarning: expect.stringContaining("not rendered"),
		});
		expect(
			renderToStaticMarkup(
				createElement(VisitHistory, {
					visits: [...experiment.items],
					state: overview.run.states[0]!,
					allStates: overview.run.states,
				}),
			),
		).toContain("Recorded facts only");
		const rejectedSnapshot = {
			branchId: "historical",
			headSeqId: f.records.find((record) => record.type === "state_action" && record.kind === "validated")!.seqId,
		};
		const rejected = await source.readStateVisits({
			runId: f.runId,
		...
		const rejectedGraph = executionGraph(overview.run, [
			{
				invokeSeqId: rejected.items[0]!.invokeSeqId,
				statePath: "experiment",
				graphStateId: "experiment",
				originBranchId: "main",
				visit: rejected.items[0]!,
			},
		]);
		expect(rejectedGraph.nodes[0]?.data).toMatchObject({ state: { status: "unknown" } });
		for (const [statePath, kind] of [
			["persistScript", "script"],
			["persistImport", "tsImport"],
		] as const) {
			...
			const graph = executionGraph(overview.run, [
				{ invokeSeqId: visit.invokeSeqId, statePath, originBranchId: "main", visit },
			]);
			expect(graph.nodes[0]?.data).toMatchObject({ displayType: kind, state: { type: kind, status: "unknown" } });
		}
		await expect(
			source.readVisitSession({ runId: f.runId, snapshot: f.snapshot, invokeSeqId: last.seqId }),
		).resolves.toMatchObject({
			status: "unknown",
			messages: [{ id: "after", text: "recorded after incompatibility" }],
		});
		await expect(
			source.readVisitSession({ runId: f.runId, snapshot: rejectedSnapshot, invokeSeqId: last.seqId }),
		).resolves.toBeUndefined();
		const full = await withRunStorage(f.storage, () =>
			hyperchartRunFromRunId(f.runId, { ast, snapshot: f.snapshot, includeTranscripts: true, readTranscript }),
		);
	...
	try {
		...
		await expect(
			withRunStorage(f.storage, () =>
				hyperchartRunFromRunId(f.runId, { ast: changedReplayScenario.ast, snapshot: f.snapshot }),
			),
		).rejects.toThrow("checkpoint IO failed");
	} finally {
		await f.store.close();
	}
});
...
it("opens the exact selected post-break visit and its transcript from the production definition-only side panel", async () => {
	const f = await fixture();
	try {
		const run = await withRunStorage(f.storage, () =>
			hyperchartRunFromRunId(f.runId, { ast: changedReplayScenario.ast, snapshot: f.snapshot }),
		);
		const invoke = [...f.records]
			.reverse()
			.find(
				(record) => record.type === "state_action" && record.kind === "invoke" && record.actionUid.state === "after",
			)!;
		const source = await withRunStorage(f.storage, () =>
			createRunInspectorDataSource(f.runId, {
				ast: changedReplayScenario.ast,
				readTranscript: async () => [
					{ id: "selected", role: "assistant", text: "exact selected durable transcript", timestamp: invoke.timestamp },
				],
			}),
		);
		...
		render(
			createElement(HyperchartInspectorSidePanel, {
				run,
				selectedStateId: "after",
				selectedInvokeSeqId: invoke.seqId,
...
it("does not accept a guarded suffix completion before its first validation or truncate the pending transcript", async () => {
	...
	try {
		...
		const invoke = f.records.find(
			(record) => record.type === "state_action" && record.kind === "invoke" && record.actionUid.state === stateId,
		);
		const claim = f.records.find(
			(record) => record.type === "state_action" && record.kind === "complete" && record.actionUid.state === stateId,
		);
		const clock = f.records.find(
			(record) =>
				record.type === "state_action" && record.kind === "complete" && record.actionUid.state === "suffix.clock.work",
		);
		const accepted = f.records.find(
			(record) => record.type === "state_action" && record.kind === "validated" && record.actionUid.state === stateId,
		);
		if (
			invoke?.type !== "state_action" ||
			invoke.kind !== "invoke" ||
			claim?.type !== "state_action" ||
			claim.kind !== "complete" ||
			clock === undefined ||
			accepted === undefined
		) {
		...
		const original = await BranchExecution.restore({
			ast: originalReplayScenario.ast,
			branchId: snapshot.branchId,
			store: f.store,
			snapshot,
			saveCheckpoint: "never",
		});
		expect(original.inspectionProjection().pendingActions).toEqual(
			expect.arrayContaining([expect.objectContaining({ actionUid: invoke.actionUid, phase: "validating" })]),
		);
		const readTranscript = vi.fn(async ({ sessionId }: { sessionId: string }) =>
				...
				? [
						{
							id: "claim",
							role: "assistant" as const,
							text: "candidate completion claimed",
							timestamp: claim.timestamp,
						},
						{
							id: "pending",
							role: "assistant" as const,
							text: "still waiting for first validation",
		...
		const source = await withRunStorage(f.storage, () =>
			createRunInspectorDataSource(f.runId, { ast: changedReplayScenario.ast, readTranscript }),
		);
		for (const headSeqId of [claim.seqId, clock.seqId]) {
			...
			expect(visit).toMatchObject({
				invokeSeqId: invoke.seqId,
				status: "unknown",
				replayWarning: expect.stringContaining("Completion claims without explicit acceptance remain unknown"),
			});
		...
		const run = await withRunStorage(f.storage, () =>
			hyperchartRunFromRunId(f.runId, { ast: changedReplayScenario.ast, snapshot }),
		);
		...
		const graph = executionGraph(run, [
			{
				...
				originBranchId: "main",
				visit: visits.items[0]!,
			},
		]);
		expect(graph.nodes.find((node) => node.id === `visit-${invoke.seqId}`)?.data).toMatchObject({
		...
		expect(acceptedVisits.items[0]).toMatchObject({
			status: "done",
			endedAt: accepted.timestamp,
			completedEvent: "DONE",
			artifactPins: [{ path: "candidate.txt", ...claim.artifacts!["candidate.txt"] }],
		});
		expect(bytes(f.runDir)).toEqual(before);
	} finally {
		await f.store.close();
