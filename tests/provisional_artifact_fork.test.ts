...
it.each([
	...
	const normalized = normalizeChartConfig(chart({ kind: "chart", id: "provisional-fork", initial: "seed", states: {
		...
		work: { kind: "state", action: agent("worker", { artifacts: { notes: artifact("notes.md") },
						validation: {
							guard: script(process.execPath, ["-e", 'let s=""; process.stdin.on("data",c=>s+=c); process.stdin.on("end",()=>process.exit(JSON.parse(s).output.accept ? 0 : 1));']),
	...
	const executor = new FakeAgentExecutor({ work: [{ type: "DONE", output: { accept: false, result: "B" } }, { type: "DONE", output: { accept: true, result: "C" } }] });
	const sourceStart = executor.start.bind(executor);
	executor.start = (effect, emit) => { writeFileSync(join(directory, "notes.md"), effect.recovery === undefined ? "B" : "C");
