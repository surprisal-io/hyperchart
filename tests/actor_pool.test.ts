...
async function effectFor(runtime: PoolRuntime, state: string, visit: number) {
	for (let turn = 0; turn < 100; turn++) {
		const effects = runtime.effects.filter(
...
function completePoolWork(runtime: PoolRuntime, effect: Extract<Effect, { kind: "agent" }>): void {
	...
	const settled = new Set(
		runtime.records.flatMap((record) =>
			record.type === "actor_message" && record.kind === "settled" && record.workerIndex === workerIndex
				? [record.messageId]
				: [],
		),
	);
	const accepted = [...runtime.records]
		.reverse()
		.find(
			(record) =>
				...
				!settled.has(record.messageId),
		);
	assert(
		accepted?.type === "actor_message" && accepted.kind === "accepted",
		`worker ${workerIndex} has no accepted message`,
	);
	const id = Number(accepted.messageId.split(":").at(-1));
	runtime.queue.send({
		kind: "agent",
		effectId: effect.id,
		outcome: { kind: "completed", event: { type: "DONE", output: { id } } },
	});
}
...
describe("static actor pools", () => {
	...
	it("durably assigns FIFO heads to eligible persistent workers and resolves callBatch in input order", async () => {
		...
		expect(Object.keys(inFlight.liveActorMessages)).toEqual([
			"work:message:1:0",
			"work:message:1:1",
			"work:message:1:2",
		]);
		expect(inFlight.actorPools["@workers"]?.mailbox).toEqual(["work:message:1:2"]);
		expect(new Set(inFlight.actorPools["@workers"]?.workers.map((worker) => worker.currentMessageId))).toEqual(
			new Set(["work:message:1:0", "work:message:1:1"]),
		);
		...
		expect(state.projection.actorPools["@workers"]).toMatchObject({
			status: "stopped",
			workers: [
				{ index: 0, currentState: "idle" },
				{ index: 1, currentState: "idle" },
			],
		});
		const accepted = runtime.records.filter(
		...
		expect(runtime.records.filter((record) => record.type === "actor_batch_call_resolved")).toEqual([
			expect.objectContaining({
				callerState: "work",
				messageIds: ["work:message:1:0", "work:message:1:1", "work:message:1:2"],
			}),
		]);
		const host = hyperchartRunFromRuntime(inspectChartAst(ast), ast, runtime.records);
		expect(host.actorOccurrences?.[0]).toMatchObject({
			kind: "actorPool",
			concurrency: 2,
			activeCount: 0,
			idleCount: 2,
			workers: [
				{ index: 0, occurrencePath: "@workers.$worker-0" },
				{ index: 1, occurrencePath: "@workers.$worker-1" },
			],
		});
		...
		const enqueueIndex = runtime.records.findIndex(
			(record) => record.type === "actor_messages_enqueued" && record.occurrence === "@workers",
		);
		assert(enqueueIndex >= 0);
		const legalNonLowest = structuredClone(runtime.records.slice(0, enqueueIndex + 1));
		const firstAccepted = runtime.records.find(
			(record) => record.type === "actor_message" && record.kind === "accepted" && record.occurrence === "@workers",
		);
		...
		const restamp = (records: DurableLogRecord[]) =>
		...
		expect(explainReplay(ast, restamp(legalNonLowest))).toMatchObject({
			prefixEnd: legalNonLowest.length,
			stale: [],
			skipped: [],
		});

		const duplicateWorker = structuredClone(runtime.records);
		const firstWave = duplicateWorker
			.filter(
				(record): record is Extract<DurableLogRecord, { type: "actor_message"; kind: "accepted" }> =>
					record.type === "actor_message" && record.kind === "accepted",
			)
		...
		const acceptedIndexes = reordered.flatMap((record, index) =>
			record.type === "actor_message" && record.kind === "accepted" ? [index] : [],
		);
		assert(acceptedIndexes[0] !== undefined && acceptedIndexes[1] !== undefined);
		[reordered[acceptedIndexes[0]], reordered[acceptedIndexes[1]]] = [
			reordered[acceptedIndexes[1]]!,
			reordered[acceptedIndexes[0]]!,
		];
		...
		const workerOneReply = replyMismatch.find(
			(record) => record.type === "actor_message" && record.kind === "replied" && record.workerIndex === 1,
		);
		...
		const lastSettlementIndex =
			premature.length -
			1 -
			[...premature].reverse().findIndex((record) => record.type === "actor_message" && record.kind === "settled");
	...
	it("uses pool-local in-flight reservations across concurrent settlements without a global append lock", async () => {
		...
		const ast = parsed(
			chart({
				kind: "chart",
				id: "pool-concurrent-settlements",
				actors: { workers },
				initial: "work",
				states: {
					work: callBatch({
		...
		expect(accepted.map((record) => record.messageId)).toEqual([
			"work:message:1:0",
			"work:message:1:1",
			"work:message:1:2",
			"work:message:1:3",
	...
	it("keeps singleton call semantics when the endpoint is a pool", async () => {
		...
		const ast = parsed(
			chart({
				kind: "chart",
				id: "pool-singleton-call",
				actors: { worker },
				initial: "call",
				states: { call: call({ to: worker, event: "WORK", input: { id: 7 }, target: "done" }), done: final() },
			}),
		);
		const runtime = new PoolRuntime(ast);
