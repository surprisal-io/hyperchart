...
function checkpoint(headSeqId: number): OpaqueCheckpointEnvelope {
	return {
		checkpointId: `checkpoint-${headSeqId}`,
		headSeqId,
...
async function exercise(
	store: LogStore & {
		loadExactCheckpoint(input: {
			targetHeadSeqId: number | null;
			selectorKey: string;
		}): Promise<OpaqueCheckpointEnvelope | undefined>;
	},
) {
	const order: string[] = [];
	...
	expect(await store.loadExactCheckpoint({ targetHeadSeqId: records[0]!.seqId, selectorKey: "opaque:test" })).toEqual(
		checkpoint(records[0]!.seqId),
	);

	await expect(
		store.appendDrafts([{ type: "args", args: { rejected: true } }], () => {
...
describe("opaque stamped-commit boundary", () => {
	...
	it("prepares before mutation and confirms after durable JSONL append", async () => {
		const root = mkdtempSync(join(tmpdir(), "hyperchart-opaque-jsonl-"));
		roots.push(root);
	...
	it.each(["memory", "jsonl"] as const)("rejects non-cloneable %s envelopes before journal mutation", async (kind) => {
		const store =
			kind === "memory"
				? new MemoryLogStore()
				: (() => {
						const root = mkdtempSync(join(tmpdir(), "hyperchart-noncloneable-"));
						roots.push(root);
	...
	it.each(["memory", "jsonl"] as const)("poisons %s after post-commit confirmation failure", async (kind) => {
		const store =
			kind === "memory"
				? new MemoryLogStore()
				: (() => {
						const root = mkdtempSync(join(tmpdir(), "hyperchart-confirm-"));
						roots.push(root);
