...
it("shares the exact session promise and never sends the invocation origin as the selected branch", async () => {
	...
	const history: RuntimeHistoryContext = {
		runId: "run",
		snapshot: { branchId: "child", headSeqId: 100 },
		dataSource: { readVisitSession } as unknown as HyperchartInspectorDataSource,
	};
	const { result, rerender } = renderHook(({ context }) => useVisitSessionReader(context), {
		initialProps: { context: history },
	});
	const top = result.current!(20);
	const lower = result.current!(20, "parent");
	expect(top).toBe(lower);
	await top;
	expect(readVisitSession).toHaveBeenCalledExactlyOnceWith({
		runId: "run",
...
it("does not retain rejected reads", async () => {
	...
	const context: RuntimeHistoryContext = {
		runId: "run",
		snapshot: { branchId: "main", headSeqId: 10 },
		dataSource: { readVisitSession } as unknown as HyperchartInspectorDataSource,
	};
