...
describe("bidirectional inspector history window", () => {
	it("deduplicates overlaps and preserves canonical newest-first order", () => {
		...
		const older = mergeHistoryWindow(
			initial,
			{
				snapshot,
	...
	it("never retains more than 1,000 rows and retains a reload cursor at the evicted edge", () => {
		...
		for (let page = 1; page <= 10; page++) {
			state = mergeHistoryWindow(
				state,
				chunk(2_000 - page * 100, 100, { older: `older-${page}`, newer: `newer-${page}` }),
				"older",
				identity,
			);
			expect(state.segments.flatMap((segment) => segment.items)).toHaveLength(
				Math.min((page + 1) * 100, HISTORY_WINDOW_ITEMS),
			);
		}
	...
	it("evicts whole opposite-edge chunks when reloading newer rows", () => {
		let state = mergeHistoryWindow(
			{ segments: [] },
			chunk(1_000, 100, { older: "o0", newer: "n0" }),
			"initial",
			identity,
		);
		for (let page = 1; page < 10; page++)
