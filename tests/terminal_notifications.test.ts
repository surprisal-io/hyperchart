...
describe("terminal notification outbox", () => {
	it("renders scoped prompts and authoritative artifact paths", () => {
		...
		const log: DurableLogRecord[] = [
			{ type: "state_action", kind: "invoke", validation: null, sessionId: "session-id", actionUid: prepareUid, definition: action(ast, "prepare"), parentId: 0, seqId: 1, branchId: "main", timestamp: 1 },
			...
			{ type: "state_action", kind: "invoke", validation: null, sessionId: "session-id", actionUid: workUid, definition: action(ast, "work"), parentId: 2, seqId: 3, branchId: "main", timestamp: 3 },
		...
		const payload = renderTerminalNotificationPayload(state, { runId: "run", branchId: "main", workDir, outcome: "complete" });
	...
	it("gates delivery on matching terminal status and keeps receipts idempotent", () => {
		...
		const payload = renderTerminalNotificationPayload(state, { runId: "run", branchId: "main", workDir: runDir, outcome: "complete" });
		withRunStorage(storage, () => patchRunStatus(runId, { branchIds: ["main"], chartId: "done", state: "starting", attemptId: "attempt-current" }));
		...
		expect(() => withRunStorage(storage, () => persistTerminalNotificationRequest(runId, { ...payload, prompt: "different" }))).toThrow(/conflict/);
		...
		expect(withRunStorage(storage, () => readDeliverableTerminalNotificationRequest(runId))?.requestId).toBe(first.requestId);

		expect(withRunStorage(storage, () => hasTerminalNotificationReceipt(runId, "pi", "session"))).toBe(false);
		const receipt = withRunStorage(storage, () => markTerminalNotificationReceipt(runId, first.requestId, "pi", "session"));
		expect(withRunStorage(storage, () => markTerminalNotificationReceipt(runId, first.requestId, "pi", "session"))).toEqual(receipt);
	...
	it("archives a failed attempt before a resumed attempt publishes success", () => {
		...
		const runDir = resolveRunPaths(runId, storage).runDir;
		mkdirSync(runDir);
		const failed = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "failed", prompt: "first attempt failed", artifacts: [], error: "stale provenance",
		}));
		...
		const receiptRelativePath = relative(join(runDir, "terminal-notification"), withRunStorage(storage, () => terminalNotificationReceiptPath(runId, failed.requestId, "pi", "session")));
		expect(existsSync(join(archiveDir!, receiptRelativePath))).toBe(true);

		const complete = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "complete", prompt: "resumed attempt completed", artifacts: [],
		}));
		expect(complete.requestId).not.toBe(failed.requestId);
		withRunStorage(storage, () => patchRunStatus(runId, { state: "complete" }));
		expect(withRunStorage(storage, () => readDeliverableTerminalNotificationRequest(runId))?.requestId).toBe(complete.requestId);
	});
	...
	it("never claims or confirms a cached request after its generation is replaced", () => {
		...
		const runDir = resolveRunPaths(runId, storage).runDir;
		mkdirSync(runDir);
		const oldRequest = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "failed", prompt: "old failure", artifacts: [], error: "old failure",
		}));
		withRunStorage(storage, () => patchRunStatus(runId, {branchIds: ["main"], chartId: "chart", state: "starting" }));
		withRunStorage(storage, () => archiveTerminalNotificationGeneration(runId));
		const newRequest = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "complete", prompt: "new success", artifacts: [],
		}));
		withRunStorage(storage, () => patchRunStatus(runId, { state: "complete" }));

		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, oldRequest.requestId, "pi", "session"))).toBe(false);
		expect(() => withRunStorage(storage, () => markTerminalNotificationReceipt(runId, oldRequest.requestId, "pi", "session"))).toThrow(/no longer active/);
		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, newRequest.requestId, "pi", "session"))).toBe(true);
		expect(withRunStorage(storage, () => hasTerminalNotificationReceipt(runId, "pi", "session"))).toBe(false);
		withRunStorage(storage, () => markTerminalNotificationReceipt(runId, newRequest.requestId, "pi", "session"));
		expect(withRunStorage(storage, () => hasTerminalNotificationReceipt(runId, "pi", "session"))).toBe(true);

		const stalePath = withRunStorage(storage, () => terminalNotificationReceiptPath(runId, oldRequest.requestId, "pi", "session"));
	...
	it("gives a recreated post-rewind outbox a fresh identity even for an identical payload", () => {
		...
		expect(withRunStorage(storage, () => persistTerminalNotificationRequest(runId, payload)).requestId).toBe(beforeRewind.requestId);
	...
	it("reclaims an unconfirmed delivery after its lease but never reclaims a confirmation", () => {
		...
		const runDir = resolveRunPaths(runId, storage).runDir;
		mkdirSync(runDir);
		const request = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "complete", prompt: "done", artifacts: [],
		}));
		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, request.requestId, "claude", "session", { now: 1_000, leaseMs: 100 }))).toBe(true);
		expect(withRunStorage(storage, () => hasTerminalNotificationReceipt(runId, "claude", "session"))).toBe(false);
		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, request.requestId, "claude", "session", { now: 1_050, leaseMs: 100 }))).toBe(false);
		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, request.requestId, "claude", "session", { now: 1_101, leaseMs: 100 }))).toBe(true);
		...
		expect(withRunStorage(storage, () => claimTerminalNotificationReceipt(runId, request.requestId, "claude", "session", { now: 10_000, leaseMs: 100 }))).toBe(false);
	});
	...
	it("recovers a stale dead run by writing the failed request before terminal status", () => {
		...
		expect(readFileSync(withRunStorage(storage, () => terminalNotificationRequestPath(runId)), "utf8")).toContain("worker crashed");
		expect(withRunStorage(storage, () => readRunStatus(runId))).toMatchObject({ state: "failed", error: "worker crashed", exitCode: 1 });
		expect(withRunStorage(storage, () => readDeliverableTerminalNotificationRequest(runId))?.requestId).toBe(request?.requestId);
	});
	...
	it("preserves a same-attempt outbox written before a hard crash and terminalizes status to its outcome", () => {
		...
		expect(withRunStorage(storage, () => recoverStaleRunTerminalNotification(runId, 20_000))?.requestId).toBe(request.requestId);
	...
	it("fails a dead resumed attempt instead of inheriting its predecessor before archival", () => {
		...
		const runDir = resolveRunPaths(runId, storage).runDir;
		mkdirSync(runDir);
		withRunStorage(storage, () => patchRunStatus(runId, {branchIds: ["main"], chartId: "chart", state: "complete", attemptId: "attempt-old" }));
		const previous = withRunStorage(storage, () => persistTerminalNotificationRequest(runId, {
			runId: "run", branchId: "main", chartId: "chart", outcome: "complete", prompt: "previous success", artifacts: [],
		}));
		withRunStorage(storage, () => patchRunStatus(runId, {
			state: "starting",
		...
		expect(withRunStorage(storage, () => readRunStatus(runId))).toMatchObject({ state: "failed", attemptId: "attempt-new", exitCode: 1 });
		expect(withRunStorage(storage, () => readDeliverableTerminalNotificationRequest(runId))?.requestId).toBe(recovered?.requestId);
	});
