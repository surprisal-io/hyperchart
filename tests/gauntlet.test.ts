			...
			for (const effect of effects) {
				switch (effect.kind) {
					...
					case "durable_records":
						...
						break;
					case "cancel":
						if (effect.requestId !== undefined && effect.target !== undefined) events.push({ kind: "cancellation_acknowledged", effectId: effect.id, requestId: effect.requestId, target: effect.target });
						break;
					case "user":
						break;
				}
