...
describe("run outcome", () => {
	it("classifies durable global failure intent as a failed run", () => {
		...
		const log: DurableLogRecord[] = [
			{ type: "state_action", kind: "invoke", validation: null, sessionId: "session-id", actionUid: uid, definition: definitionForUid(uid), parentId: 0, seqId: 1, branchId: "main", timestamp: 1 },
	...
	it("does not invent an error when a domain recovery later enters an explicit failed terminal", () => {
		...
		const log: DurableLogRecord[] = [
			{ type: "state_action", kind: "invoke", validation: null, sessionId: "session-id", actionUid: work, definition: definitionForUid(work), parentId: 0, seqId: 1, branchId: "main", timestamp: 1 },
			{ type: "state_action", kind: "complete", actionUid: work, event: { type: "NEEDS_RECOVERY" }, parentId: 1, seqId: 2, branchId: "main", timestamp: 2 },
			{ type: "state_action", kind: "invoke", validation: null, sessionId: "session-id", actionUid: recover, definition: definitionForUid(recover), parentId: 2, seqId: 3, branchId: "main", timestamp: 3 },
