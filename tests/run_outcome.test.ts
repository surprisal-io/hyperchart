...
import type { ChartAst } from "../src/core/types.js";
...
describe("run outcome", () => {
	it("classifies a terminal FAILED completion as a failed run", () => {
		...
		const log: DurableLogRecord[] = [
			{ type: "state_action", kind: "invoke", actionUid: uid, parentId: 0, seqId: 1, timestamp: 1 },
			{
				type: "state_action",
				kind: "complete",
				actionUid: uid,
	...
	it("keeps recovered workflows complete when a later success reaches done", () => {
		...
		const log: DurableLogRecord[] = [
			{ type: "state_action", kind: "invoke", actionUid: work, parentId: 0, seqId: 1, timestamp: 1 },
			{
				type: "state_action",
				kind: "complete",
				actionUid: work,
			...
			{ type: "state_action", kind: "invoke", actionUid: recover, parentId: 2, seqId: 3, timestamp: 3 },
			{
				type: "state_action",
				kind: "complete",
				actionUid: recover,
