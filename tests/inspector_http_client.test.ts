...
describe("browser inspector history transport", () => {
	it("decodes explicit absence for missing cursors and visit sessions", async () => {
		...
		await expect(source.readVisitSession({ runId: "run", snapshot: { branchId: "main", headSeqId: 9 }, invokeSeqId: 9 })).resolves.toBeUndefined();
		expect(fetchMock).toHaveBeenCalledTimes(2);
	});

	it("rejects malformed success envelopes", async () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({}) }));
		await expect(browserHistoryDataSource("token").readVisitSession({ runId: "run", snapshot: { branchId: "main", headSeqId: 9 }, invokeSeqId: 9 })).rejects.toThrow(/History request failed/);
	});
});
