...
export function answerFromReplySummary(summary: ReplySchemaSummary): unknown {
	if (summary.hasDefault && summary.defaultJson !== undefined) {
		return JSON.parse(summary.defaultJson);
	}
	if (summary.literalJson !== undefined) {
		return JSON.parse(summary.literalJson);
	}
	if ((summary.allowedValueJson?.length ?? 0) > 0) {
		return JSON.parse(summary.allowedValueJson![0]!);
	}
	...
	if (type === "array") {
		if (summary.tupleItems !== undefined) {
			return summary.tupleItems.map(answerFromReplySummary);
		}
