...
describe("model-facing tool payload boundary", () => {
	it("keeps realistic large chart and run digests below 64 KiB without full-only fields", () => {
		const chart: HyperchartInspectResult = {
			...
			states: Array.from({ length: 1_000 }, (_, index) => ({
				...
				artifacts: Array.from({ length: 25 }, (_, item) => ({
					name: `artifact-${item}`,
					path: `artifact-${item}.json`,
				})),
			...
			states: Array.from({ length: 1_000 }, (_, index) => ({
				...
				...(index === 0
					? {
							...
							issues: Array.from({ length: 25 }, (_, item) => ({
								severity: "warning" as const,
								kind: "session_failed" as const,
								message: `issue-${item}`,
								source: "session_progress" as const,
							})),
							session: {
								actionKey: "large-chart:state-0:agent",
								status: "running",
		...
		expect(chartSummary.stateDigests[0]).toMatchObject({
			omittedToolCount: 5,
			omittedReadCount: 5,
			omittedArtifactCount: 5,
			omittedTransitionCount: 5,
		});
		expect(runSummary.stateDigests[0]).toMatchObject({
			omittedArtifactCount: 5,
			omittedIssueCount: 5,
			sessionDigest: { omittedToolCount: 5 },
		});
		for (const payload of [chartSummary, runSummary]) {
			...
			expect(json).not.toMatch(
				/"(?:definitionSource|schema|schemas|states|visits|visitHistory|messages|transcripts|taskPrompt|args)"\s*:/,
			);
		}
	});
	...
	it("uses a deterministic bounded fallback after constructing the complete envelope", () => {
		...
		expect(first).toMatchObject({
			details: { error: "model-envelope-too-large", digest: expect.stringMatching(/^fnv1a32:/) },
		});
	});
	...
	it("recursively summarizes every construct needed to produce a constrained gate answer", () => {
		const summary = summarizeUserGate({
			...
			options: Array.from({ length: 25 }, (_, index) => `option-${index}`),
			...
			reply: {
				...
				schema: {
					...
					required: ["decision", "review", "findings", "comment"],
					additionalProperties: false,
				},
			},
		});
		expect(summary).toMatchObject({
			...
			options: expect.arrayContaining([
				{ label: { text: "option-0", originalChars: 8, omittedChars: 0 }, value: "option-0" },
			]),
			...
			outputHint: {
				...
				fields: [
					expect.objectContaining({
						...
						required: true,
						optional: false,
						value: { types: ["string"], allowedValueJson: ['"approve"', '"reject"'] },
					}),
					expect.objectContaining({
						name: "review",
							...
							fields: expect.arrayContaining([
								...
								expect.objectContaining({
									...
									value: expect.objectContaining({
										...
										hasDefault: true,
										defaultJson: "2",
										constraints: expect.objectContaining({ minimum: 1, maximum: 5 }),
									}),
								}),
					...
					expect.objectContaining({
						name: "findings",
						value: expect.objectContaining({
							constraints: expect.objectContaining({ minItems: 1, maxItems: 3 }),
							element: expect.objectContaining({ types: ["object"] }),
						}),
					}),
					expect.objectContaining({
						name: "comment",
		...
		expect(answer).toEqual({
			decision: "approve",
			review: { note: "xxx", priority: 2 },
			findings: [{ kind: "finding", value: "ok" }],
			comment: null,
		});
	...
	it("fails closed with omission metadata when depth, node, collection, or byte caps prevent a sufficient contract", () => {
		const summarize = (schema: Record<string, unknown>) =>
			summarizeUserGate({
				runId: "capped",
				branchId: "main",
				seqId: 1,
		...
		try {
			...
			expect((error as ReplyContractSummaryError).metadata).toEqual({
				path: "$/enum",
				limit: "collection",
				collection: "allowedValueJson",
				omittedCount: 1,
			});
		...
		expect(() =>
			summarize({
				...
				properties: Object.fromEntries(
					Array.from({ length: 161 }, (_, index) => [`field${index}`, { type: "object", properties: {} }]),
				),
			}),
		).toThrow(/collection 'fields'.*121 entries would be omitted/i);
		expect(() =>
			summarize({
				type: "object",
				properties: Object.fromEntries(
					Array.from({ length: 40 }, (_, index) => [`field${index}`, { type: "string", pattern: "a".repeat(500) }]),
				),
			}),
		).toThrow(/summary exceeds.*byte cap.*browser inspector/i);
		expect(() => summarize({ type: "string", default: "x".repeat(2_001) })).toThrow(
			/bytes would be omitted.*browser inspector/i,
		);
		expect(() => summarize({ type: "object", patternProperties: { ".*": { type: "string" } } })).toThrow(
			/Unsupported validation keyword.*browser inspector/i,
		);
	});
	...
	it("resolves local schema references and reports self-reference as recursion, not depth overflow", () => {
		...
		expect(resolved.outputHint?.fields?.[0]?.value).toMatchObject({
			types: ["string"],
			allowedValueJson: ['"approve"', '"reject"'],
		});
		...
		expect(() => summarize({ $defs: { answer: { type: "string" } }, $ref: "#/$defs/answer", minLength: 2 })).toThrow(
