...
function isBranchEntry(entry: StorageEntry): boolean {
...
describe("JsonlLogStore branch journal", () => {
	...
	it("creates main and appends stamped records with one filesystem write", async () => {
		const file = join(await makeTempDir(), "runs", "log.jsonl");
		const store = new JsonlLogStore(file);
		await store.initializeRootBranch();
	...
	it("supports multiple named movable heads and global numbering after rewind", async () => {
		...
		await main.appendDrafts([argsDraft(), invokeDraft()]);
		...
		const moved = await main.moveBranch("main", 2);
		const [replacement] = await main.appendDrafts([invokeDraft()]);
		...
		expect(
			(await persistedEntries(file)).filter((entry) => !isBranchEntry(entry)).map((record) => record.seqId),
		).toEqual([2, 3, 5, 7]);
	...
	it("trusts parsed storage entries without globally validating their sequence", async () => {
		...
		await writeFile(
			file,
			`${JSON.stringify({ kind: "branch", op: "create", seqId: 99, branchId: "main", headSeqId: null, committedAt: 1 })}\n`,
			"utf8",
		);
		const store = new JsonlLogStore(file);
	...
	it("serializes shared branch handles without reusing ids", async () => {
		...
		expect((await left.getBranch("main")).headSeqId).toBe(4);
	});
	...
	it("rejects an independently opened stale writer instead of catching it up", async () => {
		...
		expect((await stale.getBranch("main")).headSeqId).toBe(2);
		...
		expect((await persistedEntries(file)).filter((entry) => !isBranchEntry(entry)).map((entry) => entry.seqId)).toEqual(
			[2, 3],
		);
	...
	it("keeps an opened reader view stable until the journal is reopened", async () => {
		...
		const reader = new JsonlLogStore(file);
		expect(await reader.countRecords()).toBe(1);
	...
	it("concurrently appends through branch handles with global ids and independent heads", async () => {
		...
		await Promise.all([main.appendDrafts([invokeDraft()]), experiment.appendDrafts([invokeDraft()])]);
		const experimentHead = (await main.getBranch("experiment")).headSeqId;
		...
		expect((await collectHistoryRecords(main, "experiment")).map((record) => record.seqId)).toEqual([
			2,
			experimentHead,
		]);
		expect(main.fullReadCount()).toBe(1);
...
for (const backend of ["memory", "jsonl"] as const) {
	describe(`${backend} bounded history contract`, () => {
		...
		it("mints deep-link cursors and rejects snapshot, subject, and malformed cursor misuse", async () => {
			...
			const cursor = await store.cursorAt({
				snapshot,
				subject: { kind: "state-visits", state: "work" },
				seqId: target,
			});
			...
			expect(
				await store.cursorAt({ snapshot, subject: { kind: "state-visits", state: "other" }, seqId: target }),
			).toBeUndefined();
			await expect(store.readRecords({ snapshot, cursor: cursor! })).rejects.toBeInstanceOf(HistoryCursorError);
			await expect(store.readStateVisits({ snapshot, state: "work", cursor: "not-a-cursor" })).rejects.toBeInstanceOf(
				HistoryCursorError,
			);
				...
				store.appendDrafts([
					{
						...
						actionUid,
						sessionId: "invalid",
						input: { score: Number.POSITIVE_INFINITY },
						definition: { kind: "agent", uid: actionUid, name: "worker", onFail: { nudge: 2, restart: 1 } },
					},
				]),
				...
				store.appendDrafts([
					{
						...
						kind: "opened",
						actionUid: { ...actionUid, action: "user" },
						phaseSeqId: records[0]!.seqId,
						input: new Date(0) as never,
						prompt: "Choose?",
						options: [],
						events: ["OK"],
					},
		...
		it("streams projection ancestry oldest-first in fixed batches", async () => {
			...
			materialized.materializeHistoryToHead = () => {
				throw new Error("replay must not materialize full ancestry");
			};
			const actual: number[][] = [];
			...
			await expect(
				(async () => {
					for await (const _batch of openExecutionReplay(store, {
						targetHeadSeqId: records.at(-1)!.seqId,
						afterSeqId: -1,
					})) {
						/* consume */
		...
		it("returns typed actor generation and message record groups", async () => {
			...
			expect(generations.items[1]?.records.map((record) => record.seqId)).toEqual([
				records[0]!.seqId,
				records[1]!.seqId,
				records[2]!.seqId,
				records[3]!.seqId,
			...
			expect(messages.items[0]?.records.map((record) => record.seqId)).toEqual(
				records.slice(1, 5).map((record) => record.seqId),
			);
			expect(messages.items[0]?.records.map((record) => record.seqId)).toEqual([
				...new Set(messages.items[0]?.records.map((record) => record.seqId)),
			]);
			const cursor = await store.cursorAt({
				snapshot,
				subject: { kind: "actor-messages", occurrence: "@worker" },
				seqId: records[1]!.seqId,
			});
			expect(
				(await store.readActorMessages({ snapshot, occurrence: "@worker", cursor: cursor! })).items[0]?.seqId,
			).toBe(records[1]!.seqId);
		});

		it("returns typed map visit record groups", async () => {
			const store = await historyStore(backend);
			const spawned: DurableRecordDraft[] = Array.from({ length: 3 }, (_, index) => ({
				type: "spawned",
				path: "fanout",
				instances: { [`item-${index}`]: index },
			}));
...
describe("JsonlLogStore branch keyset pagination and divergent snapshots", () => {
	it("paginates branches by immutable creation coordinate", async () => {
		...
		expect(second.items).toHaveLength(100);
		expect(second.totalCount).toBe(207);
	...
	it("keeps divergent captured heads independent after branch movement", async () => {
		...
		const experimentRecords = await store
		...
		expect(
			await store.containsInHistory({ headSeqId: mainSnapshot.headSeqId, seqId: experimentRecords[0]!.seqId }),
		).toBe(false);
	});
});
