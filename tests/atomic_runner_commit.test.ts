...
afterEach(async () => {
	...
	await client.query("delete from hyperchart_journal where run_id = any($1)", [ids]);
	await client.query("delete from hyperchart_test_claims where run_id = any($1)", [ids]).catch(() => {});
	await client.end();
});
...
async function fixture(cadenceBoundary = false, continueAfterGate = false) {
	...
	await store.initializeRootBranch(undefined, {
		checkpoint: prepareProjectionCheckpoint(createBranchProjection(parsed.ast), contract, null),
	});
	await store.appendDrafts(
		Array.from({ length: cadenceBoundary ? 509 : 1 }, (_, index) => ({ type: "args", args: { index } })),
	);
...
describePg("atomic runner interaction commit", () => {
	it("unloads an idle gate without checkpoint churn and readmits after an offline response", async () => {
		...
		const checkpointCount = async () =>
			Number(
				(await client.query("select count(*)::int as count from hyperchart_checkpoint where run_id = $1", [f.runId]))
					.rows[0]!.count,
			);
	...
	it("enrolls an ordinary response before its first branch lookup so an immediate drain waits for it", async () => {
		const f = await fixture();
		let built!: () => void;
		const runtimeBuilt = new Promise<void>((resolve) => {
			built = resolve;
		});
		const controller = await createHyperchartRunnerController(
			{
		...
		await expect(controller.respondToUserInteraction("main", f.gateSeqId, { type: "SELECTED" })).rejects.toBeInstanceOf(
			BranchSealedError,
		);
	...
	it("enrolls an atomic response before its first branch lookup so an immediate drain waits for it", async () => {
		...
		let rejectedCalls = 0;
		await expect(
			controller.commitUserInteraction(
				{ branchId: "main", gateSeqId: f.gateSeqId, event: { type: "SELECTED" } },
				async () => {
					rejectedCalls++;
					return "late";
	...
	it("commits an ordinary 511-to-512 response with its due checkpoint", async () => {
		...
		const checkpoint = await client.query(
			"select 1 from hyperchart_checkpoint where run_id = $1 and head_seq_id = $2",
			[f.runId, committed.record.seqId],
		);
		await client.end();
	...
	it("re-admits an atomic response after a concurrent identical head winner", async () => {
		...
		const atomic = controller.commitUserInteraction(
			{ branchId: "main", gateSeqId: f.gateSeqId, event: { type: "SELECTED" } },
			async () => ++participants,
		);
	...
	it("never retries a participant error whose message contains moved", async () => {
		...
		await expect(
			controller.commitUserInteraction(
				...
				async () => {
					calls++;
					throw new Error("application moved: retry prohibited");
				},
			),
	...
	it("rolls back both a due response checkpoint and response when its participant fails", async () => {
		...
		).rejects.toThrow(/participant rollback/);
		const { Client } = await import("pg");
		const client = new Client({ connectionString: dsn as string });
		await client.connect();
		const rows = await client.query(
			"select record_type from hyperchart_journal where run_id = $1 and record_type = 'user_interaction' and payload->>'kind' = 'resolved'",
			[f.runId],
		);
		const checkpoints = await client.query(
			"select 1 from hyperchart_checkpoint where run_id = $1 and head_seq_id is not null",
			[f.runId],
		);
		await client.end();
		expect(rows.rows).toHaveLength(0);
		expect(checkpoints.rows).toHaveLength(0);
		await controller.stopAndDrain("main");
		await aggregate;
	}, 30_000);
	...
	it("drains only after an already tracked participant response is acknowledged and checkpointed", async () => {
		...
		let entered!: () => void;
		const participantEntered = new Promise<void>((resolve) => {
			entered = resolve;
		});
		const committing = controller.commitUserInteraction(
			{ branchId: "main", gateSeqId: f.gateSeqId, event: { type: "SELECTED" } },
		...
		await expect(controller.respondToUserInteraction("main", f.gateSeqId, { type: "SELECTED" })).rejects.toThrow(
			/draining/,
		);
		release();
		expect((await committing).participant).toBe("participant");
		expect((await drain).outcome).toBe("drained");
		await aggregate;
		...
		await client.connect();
		const rows = await client.query(
			"select max(seq) as head_seq_id from hyperchart_journal where run_id = $1 and kind = 'record' and branch_id = 'main'",
			[f.runId],
		);
	...
	it("orders an atomic user commit before the move seal or rejects it without invoking the participant", async () => {
		...
		const controller = await createHyperchartRunnerController(
			...
			() => {
				built = true;
				return new DrainingExecutor(disposalStarted, disposeGate);
			},
		);
		controller.acquireHold();
		const aggregate = controller.start();
		...
		expect(
			await reader.findUserInteractionResponse({ headSeqId: f.targetSeqId, gateSeqId: f.gateSeqId }),
		).toBeUndefined();
		const { Client } = await import("pg");
		const client = new Client({ connectionString: dsn as string });
		await client.connect();
		const move = await client.query(
			"select seq, head_seq_id from hyperchart_journal where run_id = $1 and kind = 'branch_move' and branch_id = 'main' order by seq desc limit 1",
			[f.runId],
		);
		await client.end();
		...
		const retried = await controller.forkAndCommitUserInteraction(
			...
			async (tx) => {
				const result = await tx.query(
					"select branch_id from hyperchart_test_claims where run_id = $1 and candidate = 1",
					[f.runId],
				);
