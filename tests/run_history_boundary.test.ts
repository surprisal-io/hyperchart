...
describe("bounded run-history API boundary", () => {
	it("does not expose deleted materialized-log methods or replay streams", () => {
		...
		expect(entrypoint).not.toMatch(/openProjectionReplay|PROJECTION_READ_RECORDS|NormalizedRunLog|RunLogReader|collectBranches|RespondToUserInteractionInput/);
		expect(entrypoint).not.toMatch(/\b(?:latestPinsByPath|materializeWorkspace|finalMachineFailureMessage|listHyperchartBranches)\b/);
		expect(entrypoint).toMatch(/materializeWorkspaceFromPins|listHyperchartBranchPage/);
	});
	...
	it("keeps public chunks and private replay batches capped", () => {
		...
		expect(PROJECTION_READ_RECORDS).toBe(500);
	});
	...
	it("prevents host and UI layers from importing the private replay stream", () => {
		...
		expect(consumers).not.toContain("openProjectionReplay");
