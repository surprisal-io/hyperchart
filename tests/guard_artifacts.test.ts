...
function envGuard(env: Record<string, Templatable | ArtifactOfCst | JoinArtifactOfCst>) {
	return script(
		...
		[
			"-e",
			`let s=""; process.stdin.on("data", c => s += c); process.stdin.on("end", () => { const e = JSON.parse(s); require("node:fs").writeFileSync("seen-env.json", JSON.stringify({env: process.env, event: e})); process.exit(e.type === "DONE" && !("artifacts" in e) ? 0 : 1); });`,
		],
		{ env },
	);
}
...
describe("validation script env", () => {
	it("exposes the originating invocation across rejection and distinguishes repeated visits", async () => {
		...
		const ast = parsed(
			chart({
				...
				states: {
					work: {
						kind: "state",
						action: agent("worker", { validation: { guard } }),
		...
		const seen = (await readFile(join(dir, "identities.jsonl"), "utf8"))
			.trim()
			.split("\n")
		...
		expect(seen[0]).toEqual({
			id: executor.starts[0]?.sessionId,
			branch: "main",
			action: executor.starts[0]?.actionUid,
		});
	...
	it("renders args, results, input, and visit through the same template path as script actions", async () => {
		...
		const resultChart = parsed(
			chart({
				kind: "chart",
				id: "guard-template-env",
				initial: "prepare",
				states: {
					prepare: {
						kind: "state",
						action: prepare,
						transitions: { DONE: { target: "work", input: { review: event("value") } } },
					},
					work: {
						kind: "state",
		...
		const seen = JSON.parse(await readFile(join(dir, "seen-env.json"), "utf8")) as {
	...
	it("resolves an unnamed self artifactOf against action artifacts only", async () => {
		...
		const action = agent("worker", {
			artifacts: { action: artifact("action.json") },
			validation: {
				guard: script(
					node,
					[
						"-e",
						'require("node:fs").writeFileSync("guard.json", "guard"); process.exit(process.env.SELF === "action.json" ? 0 : 1)',
					],
					{
		...
		const result = parsed(
			chart({
				kind: "chart",
				id: "guard-self-unnamed",
				initial: "work",
				states: {
					work: { kind: "state", action, transitions: { DONE: "done" } },
					done: final(),
					failed: failed(),
				},
			}),
		...
		expect(
			(await run(result.ast, dir, undefined, new FakeAgentExecutor({ work: [{ type: "DONE" }] }))).projection
				.activeLeaves,
		).toEqual(["done"]);
	});
	...
	it("supports guard artifacts and reply without turning the guard reply into an action result", async () => {
			...
			states: {
					...
					action: agent("worker", {
						validation: {
							guard: script(
								...
								[
									"-e",
									'require("node:fs").writeFileSync("diagnostic.json", JSON.stringify({approved:true})); console.log(JSON.stringify({type:"CHECKED",output:{approved:true}}))',
								],
								{ artifacts: { diagnostic: artifact("diagnostic.json", shape) }, reply: shape },
							),
						},
					}),
				...
				consume: {
					...
					action: script(
						...
						[
							"-e",
							'const fs=require("node:fs"); const v=JSON.parse(fs.readFileSync(process.env.DIAG,"utf8")); console.log(JSON.stringify({type:v.approved?"DONE":"FAILED"}))',
						],
						{ env: { DIAG: { kind: "artifactOf", state: "work", artifact: "diagnostic" } } },
					),
					transitions: { DONE: "done" },
				},
				done: final(),
		...
		const state = await run(
			...
			dir,
			undefined,
			new FakeAgentExecutor({ work: [{ type: "DONE", output: { value: 1 } }] }),
	...
	it("preserves guard env/artifacts/reply in source and inspect representations", () => {
		...
		expect(inspected?.validation?.guard).toMatchObject({
			kind: "script",
			env: [{ name: "CHECK" }],
			...
			chart({
				...
				states: {
					work: {
						kind: "state",
						action: agent("worker", {
							artifacts: { report: artifact("action.json") },
			...
			chart({
				...
				states: {
					work: {
						...
						action: agent("worker", {
							validation: {
								guard: script(node, ["-e", 'console.log(JSON.stringify({type:"CHECKED",output:{approved:true}}))'], {
									reply,
								}),
							},
						}),
	...
	it("keeps ordinary one-argument tsImport guards and plain ChartEvent stdin", async () => {
		...
		await writeFile(
			join(dir, "guard.mjs"),
			"export function check(event){ return event.type === 'DONE' && !('artifacts' in event); }\n",
			"utf8",
		);
		const resultChart = parsed(
			chart({
				...
				states: {
					work: {
						kind: "state",
						action: agent("worker", { validation: { guard: tsImport("./guard.mjs", "check") } }),
						transitions: { DONE: "done" },
					},
					done: final(),
					failed: failed(),
		...
		expect(
			(await run(resultChart.ast, dir, undefined, new FakeAgentExecutor({ work: [{ type: "DONE" }] }))).projection
				.activeLeaves,
		).toEqual(["done"]);
	});
	...
	it("rejects web URLs while letting declared artifact schemas govern data", async () => {
		...
		const schemaCheck = await checkArtifactFile(
			{
				path: "schema.json",
				shape: schema(
					z.object({ type: z.string(), properties: z.record(z.string(), z.unknown()), required: z.array(z.string()) }),
				),
			},
			dir,
		);
		...
		expect(
			(await checkArtifactFile({ path: "domain-type.json", shape: schema(z.object({ type: z.string() })) }, dir)).ok,
		).toBe(true);
		...
		expect(await resolveArtifactValue({ path: "domain.json", shape: domainShape }, dir)).toEqual({
			type: "house",
			properties: { rooms: 3 },
		});
		...
		expect(
			(
				await checkArtifactFile(
					{ path: "schema-without-root-type.json", shape: schema(z.record(z.string(), z.unknown())) },
					dir,
				)
			).ok,
		).toBe(true);
		...
		const mismatch = await checkArtifactFile(
			{ path: "domain-type.json", shape: schema(z.object({ type: z.literal("house") })) },
			dir,
		);
		expect(mismatch.ok).toBe(false);
		await expect(resolveArtifactValue({ path: "https://example.com/data.json" }, dir)).rejects.toThrow(
			"web URLs are not local artifacts",
		);
	});
});
					...
					states: {
						produce: {
							...
							action: script(
								...
								[
									"-e",
									'require("node:fs").mkdirSync("out",{recursive:true}); require("node:fs").writeFileSync("out/report.txt","x"); console.log(JSON.stringify({type:"DONE"}))',
								],
								{ artifacts: { report: artifact("out/report.txt") } },
							),
							transitions: { DONE: "check" },
						},
						check: {
							kind: "state",
							action: agent("checker", {
								validation: {
									guard: script(
										node,
										[
											"-e",
											'process.exit(JSON.parse(process.env.ALL).length === 2 && (process.env.KEY === "a" || process.env.KEY === "b") && process.env.ITEM === "{}" ? 0 : 1)',
										],
										{ env: { ALL: joinArtifactOf("items.produce"), KEY: t`${key()}`, ITEM: t`${json(item())}` } },
									),
								},
							}),
