...
function bareMapAst(): ChartAst {
	const result = normalizeChartConfig(
		chart({
			kind: "chart",
			id: "bare-map-checkpoint",
			initial: "jobs",
			states: {
				jobs: map({ over: arg("jobs"), initial: "finished", onDone: "done", states: { finished: final() } }),
				done: final(),
			},
		}) as ChartCst,
	);
...
function mapOwnedActorAst(): ChartAst {
	...
	const A = actor({
		...
		protocol: P,
		initial: "idle",
		states: { idle: receive({ on: { RUN: "reply" } }), reply: reply({ target: "idle", output: { ok: true } }) },
	});
	const endpoint = A({});
	const result = normalizeChartConfig(
		chart({
			kind: "chart",
			id: "map-owned-actor-checkpoint",
			initial: "jobs",
			states: {
				jobs: map({
					over: arg("jobs"),
					...
					initial: "run",
					onDone: "done",
					states: { run: call({ to: endpoint, event: "RUN", input: {}, target: "finished" }), finished: final() },
				}),
				done: final(),
			},
		}) as ChartCst,
...
function structuralOwnedActorAst(owner: "compound" | "parallel" | "region"): ChartAst {
	...
	const container =
		owner === "compound"
			? compound({
					...
					initial: "run",
					onDone: "done",
					states: { run: run("finished"), finished: final() },
				})
			: owner === "parallel"
				? parallel({
						actors: { endpoint },
	...
	const result = normalizeChartConfig(
		chart({
			kind: "chart",
			id: `${owner}-owned-actor-checkpoint`,
			initial: "scope",
...
function representativeAst(): ChartAst {
	const ReplyProtocol = protocol({
		RUN: message({ input: z.object({}).strict(), reply: z.object({ ok: z.boolean() }).strict() }),
	});
	const Worker = actor({
		input: z.object({}).strict(),
		...
		initial: "idle",
		states: {
			idle: receive({ on: { RUN: "reply" } }),
			reply: reply({ target: "idle", output: { ok: true } }),
		},
	});
	...
	const result = normalizeChartConfig(
		chart({
			kind: "chart",
			id: "checkpoint-representative",
			actors: { endpoint, pool },
			initial: "build",
			states: {
				build: {
					kind: "state",
					action: agent("builder", { artifacts: { report: artifact(t`report.txt`) } }),
					transitions: { BUILT: "single" },
				},
				single: call({ to: endpoint, event: "RUN", input: {}, target: "pooled" }),
				pooled: call({ to: pool, event: "RUN", input: {}, target: "ask" }),
				ask: {
					kind: "state",
					action: user({ prompt: "Approve", options: ["APPROVE"] }),
					transitions: { APPROVE: "done" },
				},
				done: final(),
			},
...
class RepresentativeRuntime implements Runtime {
	...
	constructor(
		...
		readonly store: JsonlLogStore,
		readonly pauseActorReply = false,
	) {}
	...
	async loadProjection() {
		return fullyProject(this.ast, await collectHistoryRecords(this.store, this.branchId));
	}
}

async function waitForRecord<T extends DurableLogRecord>(
	store: JsonlLogStore,
	select: (record: DurableLogRecord) => record is T,
	message: string,
): Promise<T> {
...
async function waitForOpen(
	store: JsonlLogStore,
): Promise<Extract<DurableLogRecord, { type: "user_interaction"; kind: "opened" }>> {
	return waitForRecord(
		store,
		(entry): entry is Extract<DurableLogRecord, { type: "user_interaction"; kind: "opened" }> =>
			entry.type === "user_interaction" && entry.kind === "opened",
		"representative execution did not open its gate",
	);
}
...
it("digests canonical normalized ChartAst JSON independent of object insertion order", () => {
	...
	expect(projectionContractForAst(original)).toEqual({
		projectorVersion: PROJECTOR_VERSION,
		astDigest: expect.stringMatching(/^[a-f0-9]{64}$/),
		selectorKey: expect.stringMatching(/^hyperchart-projection:/),
	});
});
...
describe("projection checkpoint schema", () => {
	it("round-trips and restores an open gate with JSON-valid resolved input", async () => {
		...
		expect(
			decodeCheckpoint(encoded, chartAst)?.projection.openUserInteractions[records.at(-1)!.seqId]?.opened.input,
		).toEqual({ context: { id: "captured" } });
	...
	it("rejects poisoned current-version nested projection families", () => {
		...
		const encoded = encodeCheckpoint({
			...
			headSeqId: null,
			contract,
			projection: createBranchProjection(chartAst),
			createdAt: 1,
		});
		const poisons: Array<(projection: Record<string, unknown>) => void> = [
			...
			(projection) => {
				projection.openUserInteractions = { 1: { status: "closed", opened: {} } };
			},
			(projection) => {
				projection.artifactPins = { report: { hash: "not-sha256", size: 1 } };
			},
			(projection) => {
				projection.actors = {
					bad: {
						declaration: "@bad",
						logicalOccurrence: "@bad",
						occurrence: "@bad",
						generation: 1,
						input: {},
						definition: {},
			...
			(projection) => {
				projection.actorPools = {
					bad: {
						...
						definition: {},
						mailbox: [],
						workers: [{ index: 0, occurrence: "@bad.$worker-0", currentState: "idle", status: "poisoned" }],
						status: "idle",
					},
				};
			},
			(projection) => {
				projection.pendingActorCalls = {
					bad: {
						kind: "singleton",
						callId: "c",
						callerState: "call",
	...
	it("isolates process-memory checkpoint reads from caller mutation", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-checkpoint-isolation-"));
		dirs.push(dir);
...
describe("projection loader", () => {
	it("accepts a bare root-map leaf checkpoint before spawn", async () => {
		const chartAst = bareMapAst();
		const store = new MemoryLogStore();
		const contract = projectionContractForAst(chartAst);
		const projection = createBranchProjection(chartAst);
		expect(projection.activeLeaves).toEqual(["jobs"]);
		await store.storeCheckpoint(
			encodeCheckpoint({ checkpointId: "bare-map", headSeqId: null, contract, projection, createdAt: 1 }),
		);
	...
	it("falls back from a matching root checkpoint with an unknown active leaf", async () => {
		...
		const store = new MemoryLogStore();
		const contract = projectionContractForAst(chartAst);
		const poisoned = createBranchProjection(chartAst);
		poisoned.activeLeaves = ["missing-state"];
		await store.storeCheckpoint(
			encodeCheckpoint({
				checkpointId: "missing-root-leaf",
				headSeqId: null,
				contract,
				projection: poisoned,
				createdAt: 1,
	...
	it("is finite-log equivalent to compacted full replay and uses batches capped at 500", async () => {
		...
		await store.appendDrafts(
			Array.from({ length: PROJECTION_CHECKPOINT_INTERVAL - 1 }, (_, index) => args(2_000 + index)),
		);
	...
	it("restores a representative execution-loop projection cold, warm, tail, and across a shared fork", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-representative-loader-"));
		dirs.push(dir);
		const chartAst = representativeAst();
		...
		runtime.queue.send({
			...
			effectId: `test:${committed.record.seqId}`,
	...
	it("falls back from invalid pending, endpoint, current-state, worker, and map coordinates", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-coordinate-poison-"));
		dirs.push(dir);
		const chartAst = representativeAst();
		const source = new JsonlLogStore(join(dir, "log.jsonl"));
		await source.initializeRootBranch();
		const runtime = new RepresentativeRuntime(chartAst, source);
		void start(runtime, {});
		await waitForOpen(source);
		const records = await collectHistoryRecords(source, "main");
		const _reference = fullyProject(chartAst, records);
		const contract = projectionContractForAst(chartAst);
		...
		const mutations: Array<(projection: MutableProjection) => void> = [
			(projection) => {
				(projection.pendingActions[0]?.actionUid as Record<string, unknown>).state = "missing-action";
			},
			(projection) => {
				projection.pendingActions[0]!.seqId = projection.seqId + 1;
			},
			(projection) => {
				projection.actors[Object.keys(projection.actors)[0]!]!.declaration = "@missing";
			},
			(projection) => {
				projection.actors[Object.keys(projection.actors)[0]!]!.currentState = "@endpoint.missing";
			},
			(projection) => {
			...
			(projection) => {
				const key = Object.keys(projection.actors)[0]!;
				projection.actors.wrong = projection.actors[key]!;
				delete projection.actors[key];
			},
		];
		for (const [index, mutate] of mutations.entries()) {
			...
			const candidate = structuredClone(seededReference) as unknown as MutableProjection;
			mutate(candidate);
			await store.storeCheckpoint(
				encodeCheckpoint({
					checkpointId: `coordinate-${index}`,
					headSeqId: seededRecords.at(-1)!.seqId,
					contract,
					projection: candidate as unknown as BranchProjection,
					createdAt: index + 1,
				}),
			);
			const loaded = await loadBranchProjection({ ast: chartAst, branchId: "main", store, contract });
	...
	it("accepts valid map-owned actors and rejects root or nonexistent-instance owners", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-map-owner-"));
		dirs.push(dir);
		const chartAst = mapOwnedActorAst();
		const source = new JsonlLogStore(join(dir, "log.jsonl"));
		await source.initializeRootBranch();
		const runtime = new RepresentativeRuntime(chartAst, source);
		await start(runtime, { jobs: { a: {} } });
		const records = await collectHistoryRecords(source, "main");
		const reference = fullyProject(chartAst, records);
		const contract = projectionContractForAst(chartAst);
		...
		for (const owner of ["arbitrary-root", "jobs#missing"]) {
			...
			const actor = Object.values(poisoned.actors)[0]!;
			const oldKey = actor.occurrence;
			actor.owner = owner;
			actor.logicalOccurrence = `${owner}.@endpoint`;
			actor.occurrence = actor.logicalOccurrence;
			delete poisoned.actors[oldKey];
			poisoned.actors[actor.occurrence] = actor;
			await store.storeCheckpoint(
				encodeCheckpoint({
					checkpointId: `owner-${owner}`,
					headSeqId: seededRecords.at(-1)!.seqId,
					contract,
					projection: poisoned,
					createdAt: 1,
	...
	it("round-trips compound, parallel, and region-owned endpoints and rejects poisoned owners", async () => {
		for (const ownerKind of ["compound", "parallel", "region"] as const) {
			const dir = await mkdtemp(join(tmpdir(), `hyperchart-${ownerKind}-owner-`));
			dirs.push(dir);
			const chartAst = structuralOwnedActorAst(ownerKind);
			const store = new JsonlLogStore(join(dir, "log.jsonl"));
			await store.initializeRootBranch();
			const runtime = new RepresentativeRuntime(chartAst, store);
			await start(runtime, {});
			...
			const actor = Object.values(poisoned.actors)[0]!;
			const oldKey = actor.occurrence;
			actor.owner = `${endpoint.owner}.missing`;
			actor.logicalOccurrence = `${actor.owner}.${actor.declaration.split(".").at(-1)}`;
			actor.occurrence = actor.logicalOccurrence;
			delete poisoned.actors[oldKey];
			poisoned.actors[actor.occurrence] = actor;
			await poisonStore.storeCheckpoint(
				encodeCheckpoint({
					checkpointId: `poison-${ownerKind}`,
					headSeqId: seededRecords.at(-1)!.seqId,
					contract,
					projection: poisoned,
					createdAt: 1,
			...
			expect(rebuilt.replayedRecords).toBe(seededRecords.length);
			expect(rebuilt.projection).toEqual(seededReference);
		}
	});

	it("round-trips a real in-flight actor call and message checkpoint", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-pending-call-loader-"));
		dirs.push(dir);
		const chartAst = representativeAst();
		const store = new JsonlLogStore(join(dir, "log.jsonl"));
		await store.initializeRootBranch();
		...
		await waitForRecord(
			store,
			(entry): entry is Extract<DurableLogRecord, { type: "actor_message"; kind: "accepted" }> =>
				entry.type === "actor_message" && entry.kind === "accepted",
			"actor call was not accepted",
		);
		const records = await collectHistoryRecords(store, "main");
		const reference = fullyProject(chartAst, records);
		...
		expect(currentMessageId === undefined ? undefined : reference.liveActorMessages[currentMessageId]?.status).toBe(
			"accepted",
		);
		...
		await poisonStore.storeCheckpoint(
			encodeCheckpoint({
				checkpointId: "missing-canonical-message",
				headSeqId: records.at(-1)!.seqId,
				contract,
				projection: poisoned,
				createdAt: 1,
	...
	it("reuses the nearest shared-ancestry checkpoint across fork and rewind", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-projection-loader-"));
		dirs.push(dir);
		const store = new JsonlLogStore(join(dir, "log.jsonl"));
		await store.initializeRootBranch();
	...
	it("ignores malformed, incompatible, and non-ancestral cache rows without mutating the journal", async () => {
		const dir = await mkdtemp(join(tmpdir(), "hyperchart-projection-loader-cache-"));
		dirs.push(dir);
		...
		const contract = projectionContractForAst(chartAst);
		await store.storeCheckpoint({
			checkpointId: "wrong-contract",
			headSeqId: mainHead!.seqId,
			selectorKey: `wrong:${contract.selectorKey}`,
			blob: {},
			createdAt: 1,
		});
		await store.storeCheckpoint({
			checkpointId: "sibling-only",
			headSeqId: siblingHead!.seqId,
			selectorKey: contract.selectorKey,
			blob: {
				schemaVersion: 1,
				projectorVersion: contract.projectorVersion,
				astDigest: contract.astDigest,
				projection: {},
			},
			createdAt: 2,
		});
		const poisonedProjection = fullyProject(chartAst, await collectHistoryRecords(store, "main")) as unknown as Record<
			string,
			unknown
		>;
		poisonedProjection.pendingActions = [{}];
		await store.storeCheckpoint({
			checkpointId: "malformed",
			headSeqId: mainHead!.seqId,
			selectorKey: contract.selectorKey,
			blob: {
				schemaVersion: 1,
	...
	it("aborts an incompatible rebuild without saving a checkpoint", async () => {
		...
		expect(
			await store.loadExactCheckpoint({ targetHeadSeqId: (await store.getBranch("main")).headSeqId, ...contract }),
		).toBeUndefined();
	});
	...
	it("rejects a caller contract that is not the supplied AST contract", async () => {
		...
		await expect(
			loadBranchProjection({
				...
				branchId: "main",
				store,
				contract: { projectorVersion: PROJECTOR_VERSION, astDigest: "0".repeat(64), selectorKey: "wrong" },
			}),
