...
const run = {
	...
	states: [
		{ id: "before", status: "done" },
		{
			...
			type: "map",
			status: "done",
			mapConfig: { visitHistory: [{ visit: 1, spawnSeqId: 1, startedAt: 1, itemCount: 2, status: "done" }] },
		},
...
describe("hierarchical execution graph", () => {
	it("keeps map actions inside parallel worker lanes and preserves top-level order", () => {
		...
		expect(
			actionNodes.every((node) =>
				node.handles?.some((handle) => handle.id === "target-top" && handle.x === 135 && handle.y === 0),
			),
		).toBe(true);
		expect(
			actionNodes.every((node) =>
				node.handles?.some((handle) => handle.id === "source-bottom" && handle.x === 135 && handle.y === 118),
			),
		).toBe(true);
		expect(graph.nodes.every((node) => node.width === node.style?.width && node.height === node.style?.height)).toBe(
			true,
		);
		const mapNode = graph.nodes.find((node) => node.type === "mapVisitGroup");
		expect(mapNode?.data).toEqual({
			state: expect.objectContaining({ id: "samples", type: "map" }),
			stateId: "samples",
			targetSeqId: 1,
		});
	...
	it("keeps parallel-region actions inside the authored parallel container", () => {
		...
		expect(parallel?.data).toEqual({
			state: expect.objectContaining({ id: "fanout", type: "parallel" }),
			stateId: "fanout",
		});
	...
	it("groups actor-internal action visits under their real actor occurrence", () => {
		...
		expect(actor?.data).toEqual({
			state: expect.objectContaining({ id: "@worker", type: "actor-occurrence" }),
			stateId: "@worker",
		});
	...
	it("creates a new map container when the same map is revisited", () => {
		...
		expect(maps.map((node) => node.id)).toEqual(["map-visit-samples-1-2", "map-visit-samples-2-8"]);
	});
});
