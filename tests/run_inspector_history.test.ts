...
describe("run inspector stateless history source", () => {
	...
	it("does not hide a failed completion after validation attempts", () => {
		...
		const invoke = { type: "state_action", kind: "invoke", actionUid, sessionId: "session", definition: { kind: "agent", uid: actionUid, name: "worker", onFail: { nudge: 2, restart: 1 } }, seqId: 1, parentId: null, branchId: "main", timestamp: 1 } as const;
