...
describe("Storybook information architecture", () => {
	...
	it("derives dialog stories from one inspected chart and durable runtime facts", () => {
		...
		expect(failedRunRecords.slice(-3).map((record) => record.type === "cancellation" ? `${record.type}:${record.kind}` : record.type)).toEqual([
			"failure_intent",
			"cancellation:requested",
			"cancellation:acknowledged",
		]);
		const request = failedRunRecords.at(-2);
		const acknowledgement = failedRunRecords.at(-1);
		if (request?.type !== "cancellation" || request.kind !== "requested" || acknowledgement?.type !== "cancellation" || acknowledgement.kind !== "acknowledged") {
			throw new Error("expected cancellation request/ack tail");
		}
		expect(acknowledgement.requestId).toBe(request.requestId);
		expect(acknowledgement.target).toEqual(request.target);
	...
	it("keeps all actor surfaces adapter-derived", () => {
		...
		expect(actorFailureRun.actorOccurrences?.[0]).toMatchObject({ status: "cancelled", cancellation: { requested: true, acknowledged: true } });
