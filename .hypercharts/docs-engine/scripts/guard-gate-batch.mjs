...
if (verdict.batchId !== batch.id) {
	violations.push(`batchId must be '${batch.id}'`);
}
if (!Array.isArray(verdict.decisions)) {
	violations.push("decisions must be an array");
}
...
for (const [index, decision] of decisions.entries()) {
	...
	if (!expectedIds.has(decision.id)) {
		violations.push(`${at}.id '${decision.id}' is not in this batch`);
	}
	if (seen.has(decision.id)) {
		violations.push(`${at}.id '${decision.id}' is duplicated`);
	}
...
for (const id of expectedIds) {
	if (!seen.has(id)) {
		violations.push(`finding '${id}' was not classified`);
	}
}
