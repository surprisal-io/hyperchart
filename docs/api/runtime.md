...
`canStartBranch(branchId)` checks current controller admission availability without reserving, waiting or replaying history. It returns false for live/already-settled reservations and temporary move fences, and throws `BranchAdmissionError` with the original cause for a failed drain. `startBranch()` still owns the authoritative fence check and synchronous reservation: a readiness check grants no lease. `onBranchChange(listener)` returns an unsubscribe function and notifies after reservation settlement and after a temporary move fence is released, including a rejected commit with no journal mutation. Notifications carry no move result or restart instruction. An external scheduler can subscribe before scanning durable unfinished work, refresh mandatory projection, and admit only eligible missing executors. It must discard decisions invalidated during asynchronous projection/capacity reads and count provisional admissions until they settle. AutoDiscovery uses this ordinary recovery loop for stopped/lost executors, including moved branches, with no move handler or restart protocol. Durable failed/terminal work is not retried; failed cleanup must surface rather than hot-loop. No machine/log facts change.
