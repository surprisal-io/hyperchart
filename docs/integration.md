...
### Inspecting an incompatible historical run

Removing only an action validator now uses ordinary replay with informational `guard_removed` warnings: the current graph may be shown, but only recorded positive verdicts accept historical guarded completions. Missing/rejected verdicts remain provisional and block resume. Legacy invokes without validation policy are also conservative; see [validator-removal recovery](safety.md#a-validator-was-removed). Other structural mismatches retain the fallback below.

When a chart edit makes the selected history structurally unreplayable (for example, changing a historical action from agent to script so its durable action identity no longer matches), run inspection recovers **read-only**, not by repairing replay. `hyperchartRunFromRunId` / `hyperchartRunOverviewFromRunId` return the production current-definition graph (`mode: "static"`, inspection status `blocked`) with `replayIncompatibility` and a durable-log `replay_warning` identifying the first broken sequence, state, and error. This is not a claim that the run was static, is complete, or is safe to resume. The inspector labels the graph **Current definition only**, hides runtime progress and Resume, and preserves the selected `historySnapshot` and branch navigation. Selecting a represented execution visit opens **Recorded history** with that exact durable visit and its **View session** control; the current-definition state header does not label the selected historical visit pending. Session steering is unavailable in recovery mode.

Snapshot-pinned record, state-visit, map, actor, and visit-session reads remain available, including the entire post-incompatibility suffix. Visit `replayWarning` labels record-only mapping: recorded inputs and invocation definitions are preserved, but templates are not rendered against the changed chart, and derived map values / scope exits are unavailable. Explicit accepted validations, failures, and timer facts remain visible. Without a trustworthy historical guard contract, a completion alone is only a recorded claim: even with zero validation records, the visit uses `status: "unknown"` and has no inferred end time, accepted event, or accepted artifact pins. Rejected-only and unresolved visits are likewise unknown, not running or successful. Raw record pages retain the completion claims; transcript reads continue through the snapshot boundary while acceptance is unknown. Actor history falls back to recorded identities/lifecycle facts without current-AST enrichment. `readVisitSession` uses the durable invocation's session binding and clips messages to its recorded end or snapshot boundary (and the next invocation sharing that session); it does not overlay live session progress in recovery mode. Pass the history data source even when the returned graph has static mode; elapsed history is not empty.

Recovery is limited to a confirmed structural replay failure matching strict restoration. Storage errors and unrelated exceptions still propagate. No log, head, checkpoint, runtime action, or replay override is written. Execution restore, `readBranchExecutionOverview`, and the runtime host adapter remain strict. Compatible overview loading keeps its existing bounded path; diagnosing a broken overview exceptionally drains snapshot-pinned record pages to confirm `explainReplay`, so very large incompatible histories cost a full ancestry scan. Historical full chart definitions are not reconstructed from incomplete action provenance.
...
### Invocation identity for custom tools and guards

`PiSessionOverridesContext.invocationId` is the existing durable action session identity, before any SDK retry-attempt suffix. It stays the same across `onReject: resume`, validation rounds, and restore; a new action visit has a new identity. Use it with `branchId` and `actionUid` for invocation-scoped tool provenance, and use the SDK tool execution context's `cwd` for the actual branch workspace.

Runtime guard contexts expose the same original identity in `context.invocation: { branchId, actionUid, invocationId }`. Script guards receive `HYPERCHART_BRANCH_ID`, `HYPERCHART_ACTION_UID` (JSON), and `HYPERCHART_INVOCATION_ID`; these override authored environment values. The identity is not the guard effect ID or the completion record sequence. Direct standalone guard calls without invocation context do not receive these variables. This is metadata exposure only: journal and replay semantics are unchanged.
