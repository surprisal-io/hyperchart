...
Projection checkpoints are disposable cache, never journal truth. A `ProjectionContract` combines the current projector/serialized-shape version with SHA-256 of canonical normalized `ChartAst` JSON. The execution-owned loader captures one head, chooses an exact or nearest shared-ancestry checkpoint, validates its contract, head membership, and versioned payload, then applies the remaining facts oldest-first in batches of at most 500 through synchronous `projectBranch()` and `compactProjection()`. Incompatible or malformed cache causes root replay without journal mutation; a replacement is written only after the complete captured head projects successfully without stale, skipped, or unpinned diagnostics (so checkpoint reuse cannot hide replay-gate findings). `ChartRuntime` tracks the replay tail and prepares a checkpoint at each 512-record boundary. PostgreSQL commits those opaque rows atomically with the journal batch; fork and rewind likewise commit their prepared target checkpoint with the branch create/move, and clean shutdown saves an exact head when the tail is non-empty. JSONL keeps checkpoints only in shared process memory, writes no sidecar, and loses them harmlessly on reopen. Branch id is not semantic checkpoint identity, so historical targets and forks reuse checkpoints from shared ancestry. Warning-bearing ancestry is marked non-checkpointable for the lifetime of its restored projection; cadence, response, shutdown, fork, and move paths therefore cannot conceal accepted stale/skipped/unpinned diagnostics. Rewind compatibility/state selection may temporarily materialize ancestry inside the private control operation; this approved interim scaffolding returns only one match and is replaced when the deferred predecessor catalog lands.
