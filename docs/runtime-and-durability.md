...
PostgreSQL does not materialize the run journal in Node. Normal opens perform no journal read. Branches, individual records, counts, and selected ancestry are targeted SQL queries; ancestry uses the record `parent_id` chain. A transaction atomically reserves ids from the run's `hyperchart_run_meta.next_seq` counter, immediately inserts its rows inside `BEGIN`, and therefore sees its own writes without rereading or replay-validating the run. Counter reservations roll back with the transaction. PostgreSQL constraints and the session advisory writer claim are the storage contract. Replay compatibility still projects the selected ancestry against the current chart; that is a semantic check, not storage-integrity validation.
