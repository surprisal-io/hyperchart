import type { ArtifactPin, DurableLogRecord, DurableRecordDraft } from "../../packages/hyperchart/src/core/durable_events.js";
...
import { originalReplayScenario, changedReplayScenario } from "../../packages/hyperchart/src/react/fixtures/replay-incompatible-fixture.js";
...
export async function captureRemovedValidatorHistory(
	...
	const state = await loop({
		...
		async *eventsQueue() { while (queue.length > 0) yield queue.shift()!; },
...
export async function captureReplayIncompatibleHistory(
		...
		async runEffects(effects: Effect[]) {
			for (const effect of effects) {
				if (effect.kind === "durable_records") {
					...
					if (added.some((record) => record.type === "state_action" && record.kind === "complete" && record.actionUid.state === "suffix.clock.work")) {
	...
	if (original.broken !== undefined || original.stale.length > 0 || original.skipped.length > 0) throw new Error("Captured history must replay with its original chart");
