...
export async function captureStorySchedule(ast: ChartAst, schedule: readonly DurableLogRecord[]): Promise<DurableLogRecord[]> {
	...
	const responses = schedule.filter((record) =>
			(record.type === "state_action" && record.kind !== "invoke") || record.type === "actor_created" || record.type === "actor_messages_enqueued"
		||
				...
				while (true) {
					...
					for (const [index, response] of responses.entries()) {
						...
						const effectIndex = effects.findIndex((effect) => {
							if (response.type === "failure_intent") return (
										"actionUid" in effect && effect.actionUid.state === response.origin && ["agent", "script", "tsImport"].includes(effect.kind)
									);
							if (response.type === "state_action") {
								if (!("actionUid" in effect) || effect.actionUid.state !== response.actionUid.state) return false;
								return response.kind === "complete" ? ["agent", "script", "tsImport"].includes(effect.kind) : response.kind === "validated" ? effect.kind === "validate" : effect.kind === "timer";
							}
							if (response.type === "actor_created") return effect.kind === "actor_create" && effect.occurrence === response.occurrence;
							if (response.type === "actor_messages_enqueued") return (
										effect.kind === "actor_enqueue" && effect.source.producerState === response.source.producerState
									);
							return (
									response.type === "actor_message" && effect.kind === "actor_reply" && effect.occurrence === response.occurrence && effect.messageId === response.messageId
								);
						});
						...
						if (response.type === "failure_intent") {
								if (effect.kind === "agent")
									...
									yield {
										kind: effect.kind,
										effectId: effect.id, event: { type: "FAILED", error: String(response.error) } };
							...
							if (response.kind === "complete") {
									if (effect.kind === "agent")
										...
										yield {
											kind: effect.kind,
											effectId: effect.id, event: response.event, ...(response.artifacts === undefined ? {} : { artifacts: response.artifacts }) };
