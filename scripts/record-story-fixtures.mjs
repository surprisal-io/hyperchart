...
const captureContext = { clock: Date.UTC(2026, 6, 14, 12), uuidSeed: "hyperchart-story-capture-v1", pipeline: "execution_loop -> explainReplay -> host adapter" };
...
function replaceRequired(source, before, after, filename) {
	if (source.split(before).length !== 2) throw new Error(`Capture transform expected exactly one ${JSON.stringify(before)} in ${filename}`);
...
const jiti = createJiti(import.meta.url, {
	...
	transform(options) {
		...
		if (families.some((family) => options.filename === `${root}${family}`)) {
			...
			source = source.replace(imports[0], `import { captureStorySchedule } from ${JSON.stringify(captureModule)};`)
			...
			if (options.filename.endsWith("/actor-fixtures.ts")) {
				...
				source = replaceRequired(source, "): HyperchartRunInfo {", "): Promise<HyperchartRunInfo> {", options.filename).replaceAll("= buildRun(", "= await buildRun(");
			}
			if (options.filename.endsWith("/specs.ts")) {
				source = replaceRequired(source, "inspectorPanelSpecInputs.map((spec) => {", "await Promise.all(inspectorPanelSpecInputs.map(async (spec) => {", options.filename);
				source = replaceRequired(source, "records: () => captured } };\n});", "records: () => captured } };\n}));", options.filename);
			}
		}
...
if (transformed.size !== families.length) throw new Error("Not every named fixture family was transformed for real capture");
const { captures } = await jiti.import(captureModule);
const { captureRemovedValidatorHistory, captureReplayIncompatibleHistory } = await jiti.import(`${root}scripts/story-fixtures/validation-histories.ts`);
for (const [name, capture] of [["removed-validator-history", captureRemovedValidatorHistory], ["replay-incompatible-history", captureReplayIncompatibleHistory]]) {
	let seqId = 0;
	const { records } = await capture(async (drafts) => drafts.map((draft) => ({ ...draft, parentId: seqId || null, seqId: ++seqId, branchId: "main", timestamp: captureContext.clock + seqId * 1_000 })));
...
const output = JSON.stringify({ captureContext, snapshots: Object.fromEntries([...captures].sort(([a], [b]) => a.localeCompare(b))) }, null, "\t") + "\n";
console.log(`${captures.size} captured snapshots, ${[...captures.values()].reduce((sum, records) => sum + records.length, 0)} records, ${Buffer.byteLength(output)} bytes`);
