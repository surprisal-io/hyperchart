...
- closing/drain progress and terminal failure status;
...
3. terminalizes run как failed;
4. отправляет best-effort runtime cancellation для pending actions.
...
- executor paths for best-effort action cancellation
...
- [x] **Build global failure infrastructure.** Add durable failure intent before switching reserved `FAILED` to global fail-fast; cancellation remains a non-durable best-effort runtime effect.
...
- Failure starts no successor and terminalizes immediately.
- Crash tests cover actor creation, enqueue, accept, action invoke/complete, reply validation/settlement, caller wake-up, closing and failure intent.
